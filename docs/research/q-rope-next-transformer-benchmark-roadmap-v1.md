...
The next gate should redesign the stronger matched decoder-only transformer beyond Stage 54 enough to generalize on retrieval tasks, then rerun the same fair-comparison frame. Hardware witness hardening remains a separate replication track and should not displace the fair-comparison promotion path.
