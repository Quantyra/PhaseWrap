...
The next gate should redesign the stronger matched decoder-only transformer beyond Stage 68 enough to improve original phase-cued/exact-offset held-out retrieval generalization before testing positional-method promotion. Hardware witness hardening remains a separate replication track and should not displace the fair-comparison promotion path.
