...
Stage 68 trains original tasks with content-key auxiliary rows. It still does not satisfy this promotion gate because the auxiliary signal does not transfer into original held-out phase-cued or exact-offset retrieval generalization.

Stage 69 trains all original tasks together. It still does not satisfy this promotion gate because shared original-task training preserves capacity but does not repair original held-out retrieval generalization.

Stage 70 synthesizes the current evidence boundary. It still does not satisfy this promotion gate because it is a claim-boundary artifact, not new model evidence, and it records `BOUND_STRONGEST_HONEST_CLAIM_WITH_RETRIEVAL_FAILURES`.

Stage 71 applies deterministic positional-bias argmax copy on the original rows. It still does not satisfy this promotion gate because hard copy is not learned decoder-only transformer behavior and because phase-cued retrieval remains below threshold for every method.

Stage 72 makes that phase-cued diagnostic tie-aware. It still does not satisfy this promotion gate because PhaseWrap max-bias support misses the held-out phase-cued target, and `no_position` support is only ambiguous all-prefix support.
...
Stage 77 adds auxiliary support supervision to the integrated copy objective and records `AUXILIARY_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`: exact-offset remains partially repaired for `rope_relative`, but phase-cued retrieval remains `0.000000` for every method.

Stage 78 audits support split coverage and records `SAME_SEED_SUPPORT_COVERAGE_SPLIT_EXPLAINS_STAGE77_FAILURE`: default same-seed train rows provide zero held-out phase-cued support coverage, while cross-seed and pooled train rows provide full coverage.

Stage 79 restores same-seed support-complete exposure and records `SUPPORT_COMPLETE_AUXILIARY_COPY_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`: support labels are recovered, but phase-cued token retrieval still fails.
