...
- Stage 74 learns visible query-support maps from other seeds' train rows and records `LEAVE_ONE_SEED_QUERY_SUPPORT_SOLVES_PHASE_CUED_NOT_PROMOTION`; phase-cued retrieval reaches top-1 `0.900000`, but `no_position` reaches the same result.
- Stage 75 replaces the Stage 74 hard lookup with a learned visible query-support head and records `LEARNED_QUERY_SUPPORT_HEAD_SOLVES_PHASE_CUED_NOT_PROMOTION`; phase-cued retrieval reaches top-1 `0.900000`, but `no_position` reaches the same result.
- Stage 76 trains an integrated support/copy head through token-copy loss and records `INTEGRATED_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; exact-offset reaches top-1 `0.600000` for `rope_relative`, but phase-cued retrieval is `0.000000` for every method.
- Stage 77 adds auxiliary support supervision to the integrated support/copy head and records `AUXILIARY_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; exact-offset remains partially repaired for `rope_relative`, but phase-cued retrieval is still `0.000000` for every method.
- Stage 78 audits the support split and records `SAME_SEED_SUPPORT_COVERAGE_SPLIT_EXPLAINS_STAGE77_FAILURE`; same-seed train support has held-out phase-cued coverage `0.000000`, while cross-seed and pooled train support have coverage `1.000000`.
- Stage 79 restores same-seed support-complete exposure and records `SUPPORT_COMPLETE_AUXILIARY_COPY_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; mean phase-cued support accuracy reaches `1.000000`, but best phase-cued token retrieval remains only top-1 `0.016667`.
- Stage 80 routes recovered support into farthest-congruent token selection and records `SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; phase-cued retrieval reaches top-1 `1.000000`, but every method solves, including `no_position`.
- Stage 81 routes learned support probabilities into farthest-congruent token selection and records `SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; phase-cued retrieval remains top-1 `1.000000` for every method, including `no_position`.
- Stage 82 trains a compact support-to-token routing head and records `LEARNED_SUPPORT_ROUTING_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support accuracy remains `1.000000`, but best phase-cued top-1 is only `0.333333`.
- Stage 83 trains a nonlinear support-routing bridge and records `NONLINEAR_SUPPORT_ROUTING_BRIDGE_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support accuracy remains `1.000000`, but best phase-cued top-1 is only `0.283333`.
- Stage 84 trains a support-auxiliary two-block pointer-generator decoder and records `SUPPORT_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`; train capacity is preserved, but best phase-cued top-1 is only `0.016667`.
- Stage 85 trains a dual support/target-attention auxiliary pointer-generator decoder and records `DUAL_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`; train capacity is preserved, tiny-text QA reaches top-1 `0.933334`, exact-offset reaches top-1 `0.416667`, and phase-cued retrieval remains only top-1 `0.050000`.
- Stage 86 runs a practical budget-sensitivity audit over the Stage 85 dual-auxiliary path and records `DUAL_AUXILIARY_BUDGET_WITHOUT_RETRIEVAL_GENERALIZATION`; extending from `10` to `20` epochs does not repair held-out retrieval.
- Stage 87 adds a structural in-decoder support-routed copy expert and records `IN_DECODER_SUPPORT_ROUTED_COPY_EXPERT_SOLVES_PHASE_CUED_NOT_PROMOTION`; phase-cued retrieval reaches top-1 `0.783333`, but `no_position` also solves and exact-offset remains below threshold at top-1 `0.416667`.
...
- a claim that Stage 75's standalone support head is a matched decoder-only transformer, proves PhaseWrap-specific positional advantage, or supports RoPE replacement.
...
- a claim that Stage 85 dual support/target-attention supervision repairs held-out retrieval, validates a production language model, or supports RoPE replacement.
- a claim that Stage 86 budget tuning repairs held-out retrieval, validates a production language model, or supports RoPE replacement.
- a claim that Stage 87 structural support-routed expert success is standard decoder-only language modeling, positional-method promotion, or RoPE replacement evidence.
