# PhaseWrap Quickstart and Results Summary v1
...
This page is the one-page reviewer entry point for the current PhaseWrap negative-results release. It summarizes what is present, what passes, what failed, what is excluded, and how to reproduce the evidence.

Repository naming note: public materials use `PhaseWrap`; internal imports and artifact IDs retain the existing `qrope` stem.

## Current Result

The positive replacement research line is closed. The repository should be read as a negative-results and methodology package: fixed-period PhaseWrap is compact and auditable, but the evidence does not support RoPE replacement, production transformer superiority, quantum advantage, or broad cross-backend robustness.
...
Run the Stage 63 no-credential two-block copy-output capacity audit:

```bash
python scripts/run_stage63_two_block_copy_output_capacity_audit.py
```

This writes `logs/automated_stage_gates/stage63_two_block_copy_output_capacity_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `TWO_BLOCK_COPY_OUTPUT_CAPACITY_WITHOUT_RETRIEVAL_GENERALIZATION`: copy output establishes train capacity, with best train top-1 `1.000000`, but both retrieval lanes remain at best held-out top-1 `0.050000`.

Run the Stage 64 no-credential two-block learned pointer-generator capacity audit:

```bash
python scripts/run_stage64_two_block_pointer_generator_capacity_audit.py
```

This writes `logs/automated_stage_gates/stage64_two_block_pointer_generator_capacity_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `TWO_BLOCK_POINTER_GENERATOR_CAPACITY_WITHOUT_RETRIEVAL_GENERALIZATION`: the learned copy/vocab mixture preserves train capacity, with best train top-1 `1.000000`, but phase-cued retrieval reaches only `0.016667` and exact-offset passkey reaches only `0.050000` held-out top-1.

Run the Stage 65 no-credential pointer-generator length-curriculum audit:

```bash
python scripts/run_stage65_pointer_generator_length_curriculum_audit.py
```

This writes `logs/automated_stage_gates/stage65_pointer_generator_length_curriculum_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `POINTER_GENERATOR_LENGTH_CURRICULUM_WITHOUT_RETRIEVAL_GENERALIZATION`: adding length-40 rows preserves train capacity and improves tiny text-fact QA to `0.783333` best held-out top-1, but phase-cued retrieval and exact-offset passkey each reach only `0.033333` best held-out top-1.

Run the Stage 66 no-credential positional-copy expert audit:

```bash
python scripts/run_stage66_positional_copy_expert_audit.py
```

This writes `logs/automated_stage_gates/stage66_positional_copy_expert_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `POSITIONAL_COPY_EXPERT_WITHOUT_RETRIEVAL_GENERALIZATION`: a direct method-bias copy expert preserves train capacity, with best train top-1 `1.000000`, but phase-cued retrieval reaches only `0.033333` and exact-offset passkey reaches only `0.050000` best held-out top-1.

Run the Stage 67 no-credential content-key retrieval audit:

```bash
python scripts/run_stage67_content_key_retrieval_audit.py
```

This writes `logs/automated_stage_gates/stage67_content_key_retrieval_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `CONTENT_KEY_RETRIEVAL_SOLVABLE_FOR_ALL_METHODS_NOT_PROMOTION`: redesigned visible content-key retrieval reaches held-out top-1 `1.000000` and MRR `1.000000` for every tested method, including `no_position`.

Run the Stage 68 no-credential content-key auxiliary transfer audit:

```bash
python scripts/run_stage68_content_key_auxiliary_transfer_audit.py
```

This writes `logs/automated_stage_gates/stage68_content_key_auxiliary_transfer_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `CONTENT_KEY_AUXILIARY_TRANSFER_WITHOUT_RETRIEVAL_GENERALIZATION`: content-key auxiliary training preserves train capacity, but phase-cued retrieval reaches only `0.016667` and exact-offset passkey only `0.033333` best held-out top-1.

Run the Stage 69 no-credential original multitask pointer-generator audit:

```bash
python scripts/run_stage69_original_multitask_pointer_generator_audit.py
```

This writes `logs/automated_stage_gates/stage69_original_multitask_pointer_generator_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `ORIGINAL_MULTITASK_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`: original-task multitask training preserves capacity, but phase-cued retrieval reaches only `0.016667` and exact-offset passkey only `0.033333` best held-out top-1.

Run the Stage 70 no-credential strongest honest claim synthesis:

```bash
python scripts/run_stage70_strongest_honest_claim_synthesis.py
```

This writes `logs/automated_stage_gates/stage70_strongest_honest_claim_synthesis/manifest.json`, `results.json`, and `summary.csv`. The synthesis records `BOUND_STRONGEST_HONEST_CLAIM_WITH_RETRIEVAL_FAILURES`: PhaseWrap is a compact, auditable RoPE-adjacent positional rule with mixed diagnostics, but current fair matched evidence does not support RoPE replacement.

Run the Stage 71 no-credential positional-bias copy upper-bound audit:

```bash
python scripts/run_stage71_positional_bias_copy_upper_bound_audit.py
```

This writes `logs/automated_stage_gates/stage71_positional_bias_copy_upper_bound_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, `per_example_results.csv`, and `failed_runs.json`. The audit records `POSITIONAL_BIAS_COPY_PARTIAL_ORIGINAL_RETRIEVAL_UPPER_BOUND`: deterministic positional-bias copy solves exact-offset passkey for `rope_relative`, but phase-cued retrieval remains below threshold for every method.

Run the Stage 72 no-credential phase-cued bias tie-support audit:

```bash
python scripts/run_stage72_phase_cued_bias_tie_support_audit.py
```

This writes `logs/automated_stage_gates/stage72_phase_cued_bias_tie_support_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, `per_example_results.csv`, and `failed_runs.json`. The audit records `PHASE_CUED_TARGET_NOT_IN_PHASEWRAP_MAX_BIAS_SUPPORT`: PhaseWrap max-bias support does not contain the held-out phase-cued target, while `no_position` contains it only through ambiguous all-prefix support.

Run the Stage 73 no-credential phase-cued period-pair support audit:

```bash
python scripts/run_stage73_phase_cued_period_pair_support_audit.py
```

This writes `logs/automated_stage_gates/stage73_phase_cued_period_pair_support_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, `per_example_results.csv`, and `failed_runs.json`. The audit records `PHASE_CUED_PERIOD_PAIR_SWEEP_DOES_NOT_REPAIR_SUPPORT`: every predeclared fixed PhaseWrap period pair has held-out target-in-max-support rate `0.000000`; the best top-1 is `12/24` at `0.033333`.

Run the Stage 74 no-credential leave-one-seed query-support audit:

```bash
python scripts/run_stage74_leave_one_seed_query_support_audit.py
```

Run the Stage 75 no-credential learned query-support head audit:

```bash
python scripts/run_stage75_learned_query_support_head_audit.py
```

Run the Stage 76 no-credential integrated support/copy-head audit:

```bash
python scripts/run_stage76_integrated_support_copy_head_audit.py
```

Run the Stage 77 no-credential auxiliary support/copy-head audit:

```bash
python scripts/run_stage77_auxiliary_support_copy_head_audit.py
```

Run the Stage 78 no-credential support-coverage split audit:

```bash
python scripts/run_stage78_support_coverage_split_audit.py
```

Run the Stage 79 no-credential support-complete auxiliary support/copy-head audit:

```bash
python scripts/run_stage79_support_complete_auxiliary_copy_head_audit.py
```

Run the Stage 80 no-credential support-routed token selector audit:

```bash
python scripts/run_stage80_support_routed_token_selector_audit.py
```

This writes `logs/automated_stage_gates/stage80_support_routed_token_selector_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`: support-routed farthest-congruent token selection repairs phase-cued retrieval for every method, including `no_position`.

Run the Stage 81 no-credential soft support-routed token selector audit:

```bash
python scripts/run_stage81_soft_support_routed_token_selector_audit.py
```

This writes `logs/automated_stage_gates/stage81_soft_support_routed_token_selector_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`: learned support probabilities can replace hard support argmax, but the farthest-congruent routing repair still solves for every method, including `no_position`.

Run the Stage 82 no-credential learned support-routing head audit:

```bash
python scripts/run_stage82_learned_support_routing_head_audit.py
```

This writes `logs/automated_stage_gates/stage82_learned_support_routing_head_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `LEARNED_SUPPORT_ROUTING_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`: support labels recover, but a compact learned routing head does not repair phase-cued retrieval.

Run the Stage 83 no-credential nonlinear support-routing bridge audit:

```bash
python scripts/run_stage83_nonlinear_support_routing_bridge_audit.py
```

This writes `logs/automated_stage_gates/stage83_nonlinear_support_routing_bridge_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `NONLINEAR_SUPPORT_ROUTING_BRIDGE_SUPPORT_RECOVERED_RETRIEVAL_FAILED`: support labels recover, but a nonlinear per-position routing bridge still does not repair phase-cued retrieval.

Run the Stage 84 no-credential support-auxiliary pointer-generator audit:

```bash
python scripts/run_stage84_support_auxiliary_pointer_generator_audit.py
```

This writes `logs/automated_stage_gates/stage84_support_auxiliary_pointer_generator_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `SUPPORT_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`: in-decoder support supervision preserves train capacity but does not repair held-out retrieval.

This writes `logs/automated_stage_gates/stage74_leave_one_seed_query_support_audit/manifest.json`, `results.json`, `summary.csv`, `per_run_results.csv`, and `failed_runs.json`. The audit records `LEAVE_ONE_SEED_QUERY_SUPPORT_SOLVES_PHASE_CUED_NOT_PROMOTION`: phase-cued retrieval reaches top-1 `0.900000`, but `no_position` reaches the same result.
...
- A learned Stage 63 two-block copy-output audit showing that copied prefix-token mass establishes train capacity, while held-out retrieval still does not generalize.
- A learned Stage 64 two-block pointer-generator audit showing that a learned copy/vocab mixture preserves train capacity, while held-out retrieval still does not generalize.
- A learned Stage 65 pointer-generator length-curriculum audit showing that adding length-40 rows preserves capacity and improves tiny text-fact QA, while held-out retrieval still does not generalize.
- A learned Stage 66 positional-copy expert audit showing that a direct method-bias copy expert preserves capacity, while held-out retrieval still does not generalize.
- A learned Stage 67 content-key retrieval audit showing that the current two-block pointer-generator can solve standard visible content-key retrieval at held-out lengths, while also showing the result is not PhaseWrap-specific because `no_position` solves too.
- A learned Stage 68 content-key auxiliary transfer audit showing that the Stage 67 content-key signal does not transfer back to original phase-cued/exact-offset held-out retrieval generalization.
- A learned Stage 69 original multitask pointer-generator audit showing that shared training across original tasks still does not repair phase-cued/exact-offset held-out retrieval generalization.
- A Stage 70 strongest-honest-claim synthesis showing that the current fair evidence supports a bounded compact/auditable PhaseWrap claim, not RoPE replacement or positional-method promotion.
- A deterministic Stage 71 positional-bias copy upper-bound audit showing that exact-offset passkey is directly reachable for `rope_relative`, while phase-cued retrieval remains unsolved even under hard copy from the method-bias argmax.
- A deterministic Stage 72 phase-cued tie-support audit showing that PhaseWrap max-bias support does not contain the held-out phase-cued target, so the blocker is not only hard-argmax tie selection.
- A deterministic Stage 73 period-pair support audit showing that the phase-cued support miss is not repaired by the predeclared fixed PhaseWrap period-pair grid.
- A deterministic Stage 74 leave-one-seed query-support audit showing that visible support recovery can solve phase-cued retrieval from other seeds' train rows, but not in a PhaseWrap-specific way.
- A learned Stage 75 query-support head audit showing that visible support recovery can be learned by a standalone head, but still solves for `no_position` too.
- A learned Stage 76 integrated support/copy-head audit showing that phase-cued retrieval returns to zero when the support cue is optimized through the token-copy objective.
- A learned Stage 77 auxiliary support/copy-head audit showing that explicit support supervision still does not preserve phase-cued held-out retrieval.
- A deterministic Stage 78 support-coverage split audit showing that default same-seed train support has zero held-out phase-cued coverage, while cross-seed and pooled train support have full coverage.
- A learned Stage 79 support-complete auxiliary support/copy-head audit showing that support labels recover but phase-cued token retrieval still fails.
- A Stage 80 support-routed token selector audit showing that recovered support can repair phase-cued retrieval when hard-routed into farthest-congruent token selection, but the repair also solves for `no_position`.
- A Stage 81 soft support-routed token selector audit showing that learned support probabilities preserve the phase-cued repair, but the repair still solves for `no_position`.
- A Stage 82 learned support-routing head audit showing that support recovery is preserved, but a compact learned routing head does not preserve the phase-cued repair.
- A Stage 83 nonlinear support-routing bridge audit showing that support recovery is preserved, but a nonlinear per-position routing bridge still does not preserve the phase-cued repair.
- A Stage 84 support-auxiliary pointer-generator audit showing that in-decoder support supervision preserves train capacity but still does not repair held-out retrieval.
- An active claim-boundary research goal: find the strongest honest claim PhaseWrap can support under fair RoPE, ALiBI, sinusoidal, and no-position comparisons while preserving both positive evidence and failure modes.
...
- a claim that Stage 8 is a standard RULER benchmark, production transformer result, or proof that PhaseWrap replaces RoPE.
- a claim that Stage 9 is a full language-model benchmark, production transformer result, or proof that PhaseWrap replaces RoPE.
...
- a claim that Stage 12 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 13 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 14 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 15 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 16 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 17 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 18 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 19 establishes production transformer superiority, compares positional mechanisms, or proves that PhaseWrap replaces RoPE.
- a claim that Stage 20 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 21 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 22 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 23 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 24 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 25 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 26 establishes production transformer superiority or that PhaseWrap replaces RoPE.
- a claim that Stage 27 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 28 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 29 proves a globally optimal period pair or that period-pair swaps alone make PhaseWrap a RoPE replacement.
- a claim that Stage 30 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 31 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 32 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 33 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 34 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 35 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 36 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 37 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 38 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 39 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 40 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 41 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 42 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
- a claim that Stage 43 establishes production transformer superiority, full decoder-only language-model validation, solved free value generation, or that PhaseWrap replaces RoPE.
...
- a claim that Stage 45 establishes production transformer superiority, full decoder-only language-model validation, or that PhaseWrap replaces RoPE.
...
- a claim that Stage 47 establishes production transformer superiority, full decoder-only language-model validation, or that tiny text-fact QA alone validates PhaseWrap as a replacement.
- a claim that the Stage 47 one-seed PhaseWrap tiny text-fact QA lead is stable across seeds.
- a claim that Stage 49 establishes production transformer superiority, full decoder-only language-model validation, free learned value generation, or that PhaseWrap replaces RoPE.
- a claim that Stage 50 establishes production transformer superiority, full decoder-only language-model validation, learned retrieval generation, or that PhaseWrap replaces RoPE.
...
- a claim that Stage 66 is positional-method promotion evidence, that a direct positional-copy expert is full decoder-only language-model validation, or that output-path expertise establishes held-out retrieval generalization.
- a claim that Stage 67 is positional-method promotion evidence, that content-key retrieval is equivalent to the original phase-cued/exact-offset gate, or that all-method content-key solvability supports a PhaseWrap-specific replacement claim.
- a claim that Stage 68 is positional-method promotion evidence, that content-key auxiliary training solves the original phase-cued/exact-offset gate, or that auxiliary transfer supports a PhaseWrap-specific replacement claim.
- a claim that Stage 69 is positional-method promotion evidence, that original multitask training is equivalent to a larger decoder-only language model, or that shared original-task training solves held-out retrieval generalization.
- a claim that Stage 70 validates RoPE replacement, converts diagnostic positives into original retrieval generalization, or closes the transformer-promotion case.
- a claim that Stage 71 is learned decoder-only transformer behavior, solves phase-cued retrieval, or promotes any positional method by itself.
...
- a claim that Stage 74 is a matched decoder-only transformer, proves PhaseWrap-specific positional advantage, or supports RoPE replacement.
- a claim that Stage 75's standalone learned support head is a matched decoder-only transformer, proves PhaseWrap-specific positional advantage, or supports RoPE replacement.
- a claim that Stage 76 preserves learned phase-cued recovery, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 77 auxiliary support supervision repairs phase-cued retrieval, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 78 support coverage alone validates a matched decoder-only transformer, proves PhaseWrap-specific positional advantage, or supports RoPE replacement.
- a claim that Stage 79 support-complete auxiliary copy training repairs phase-cued token retrieval, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 80 support-routed token selection is positional-method promotion, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 81 soft support-routed token selection is positional-method promotion, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 82 learned support routing repairs phase-cued retrieval, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 83 nonlinear support routing repairs phase-cued retrieval, validates a matched decoder-only transformer, or supports RoPE replacement.
- a claim that Stage 84 in-decoder support supervision repairs held-out retrieval, validates a production language model, or supports RoPE replacement.
...
- **Does PhaseWrap help a classical ML task?** Stage 5 through Stage 84 are now present as bounded synthetic downstream checks and synthesis artifacts. Stage 9 gives a compact trained positional-attention result with a useful split: PhaseWrap wins the phase-cued lane, while RoPE-relative wins the exact-offset passkey lane. Stage 10 adds a very small decoder-only transformer run, but it is near chance. Stage 12 adds a stricter non-phase-cued retrieval packet where RoPE-like and sinusoidal baselines solve the task and the fixed PhaseWrap score does not. Stages 13-43 preserve a mixed pattern: PhaseWrap-derived adapters can recover ranking in selected compact settings, while RoPE-like scoring often keeps stronger probability/calibration and the learned value-generation path remains weak. Stage 44 records the compact sequence and pointer-generator diagnostics as a bounded plateau. Stage 45 then runs a matched one-block decoder-only gate and records `PROMOTION_NOT_SUPPORTED`; Stage 46 confirms longer training still leaves the harness below capacity threshold; Stage 47 shows Adam solves train fit and gives a one-seed PhaseWrap-positive tiny text-fact QA result; Stage 48 shows that lead is not stable across five seeds and retrieval generalization still fails. Stage 49 shows copy-output repair can solve exact-offset passkey for RoPE-relative scoring, Stage 50 shows that repair does not survive learned pointer-generator attention/gating, Stage 51 records the decoder path as bounded, Stage 52 starts a stronger two-block decoder path, Stage 53 shows simple hardening of that path still fails retrieval, Stage 54 shows target-attention supervision still does not repair held-out retrieval attention or top-1, Stage 55 shows the row family is solvable only when explicit cue metadata is handed to all methods, Stage 56 shows visible-token cue decoding still does not solve phase-cued retrieval, Stage 57 shows a known support prior recovers phase-cued retrieval for `no_position` too, Stage 58 shows that support map is train-recoverable but still not positional-method evidence, Stage 59 shows seed-local held-out support coverage is incomplete while fallback cue decoding still crosses threshold for `no_position`, Stage 60 shows that strict known-support decoding does not preserve the phase-cued threshold crossing, Stage 61 shows complete support coverage still does not make the learned two-block decoder fit, Stage 62 shows longer training improves fit but still misses capacity and held-out retrieval, Stage 63 shows copy output establishes train capacity but still misses retrieval generalization, Stage 64 shows learned pointer-generation preserves capacity but still misses retrieval generalization, Stage 65 shows adding length-40 curriculum rows preserves capacity and improves tiny text-fact QA while still missing retrieval generalization, Stage 66 shows a direct positional-copy expert still misses retrieval generalization, Stage 67 shows standard content-key retrieval solves for all methods including `no_position`, Stage 68 shows content-key auxiliary training still does not repair original retrieval generalization, Stage 69 shows original-task multitask training still does not repair original retrieval generalization, Stage 70 records the bounded strongest honest claim with those failures preserved, Stage 71 shows deterministic positional-bias copy solves exact-offset for `rope_relative` but not phase-cued retrieval, Stage 72 shows PhaseWrap max-bias support misses the held-out phase-cued target, Stage 73 shows the predeclared fixed period-pair grid does not repair that support miss, Stage 74 shows cross-seed visible query-support recovery solves phase-cued retrieval for `no_position` too, Stage 75 shows a learned support head can internalize that cue but still solves for `no_position` too, Stage 76 shows integrated support/copy training loses the phase-cued repair, Stage 77 shows auxiliary support supervision still leaves phase-cued retrieval at zero, Stage 78 explains that same-seed train support coverage is zero under the default split, Stage 79 shows support-complete exposure recovers support labels but not phase-cued token retrieval, Stage 80 shows hard support-to-token routing repairs phase-cued retrieval for every method including `no_position`, Stage 81 shows the repair survives learned support probabilities but still is not method-specific, Stage 82 shows a compact learned routing head does not preserve that repair, Stage 83 shows a nonlinear support-routing bridge still does not preserve it, and Stage 84 shows in-decoder support supervision still does not repair held-out retrieval.
- **What would make the RoPE-replacement case stronger?** The next expansion should redesign the stronger matched small decoder-only transformer beyond compact support/copy heads with RoPE, ALiBI, sinusoidal, no-position, and PhaseWrap positional mechanisms; evaluate train-short/test-long context extrapolation; include non-synthetic retrieval or QA tasks; run at least five seeds; and publish failed runs plus confidence intervals. Stage 12 and Stage 13 make clear that the fixed score alone is not enough; Stage 55 makes clear that solving the rows with explicit metadata is not positional-method evidence; Stage 62 makes clear that longer training alone is not enough; Stages 63 and 64 make clear that output-path capacity alone is not enough; Stage 65 makes clear that simple length curriculum alone is not enough; Stage 66 makes clear that a direct positional-copy expert alone is not enough; Stage 67 makes clear that content-key solvability for all methods is not PhaseWrap-specific promotion; Stage 68 makes clear that content-key auxiliary transfer does not solve the original retrieval blocker; Stage 69 makes clear that original-task multitask training alone does not solve the blocker; Stage 70 makes clear that the current strongest honest claim remains bounded; Stage 71 makes clear that hard positional-bias copy still does not solve phase-cued retrieval; Stage 72 makes clear that PhaseWrap max-bias support misses the held-out phase-cued target; Stage 73 makes clear that fixed period-pair selection over the predeclared grid does not repair the support miss; Stage 74 makes clear that deterministic visible query-support recovery is not PhaseWrap-specific promotion; Stage 75 makes clear that standalone learned visible query-support recovery is still not PhaseWrap-specific promotion; Stage 76 makes clear that integrated support/copy training does not preserve phase-cued recovery; Stage 77 makes clear that auxiliary support supervision still does not preserve phase-cued recovery; Stage 78 makes clear that the default same-seed split has zero held-out support coverage; Stage 79 makes clear that support recovery alone still does not repair phase-cued token retrieval; Stage 80 makes clear that hard support-to-token routing is still not PhaseWrap-specific promotion; Stage 81 makes clear that soft learned support routing is also not PhaseWrap-specific promotion; Stage 82 makes clear that compact learned routing still does not repair phase-cued retrieval; Stage 83 makes clear that a nonlinear support-routing bridge still does not repair phase-cued retrieval; Stage 84 makes clear that in-decoder support supervision still does not repair held-out retrieval. The detailed gate is tracked in `docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md`.
...
| Stage 7 | Four-layer toy transformer ablation | Complete for one fixed synthetic length-extrapolation packet; PhaseWrap has the best argmax ranking, while calibration remains mixed. |
...
| Stage 63 | Two-block copy-output capacity audit | Complete across five seeds. Decision: `TWO_BLOCK_COPY_OUTPUT_CAPACITY_WITHOUT_RETRIEVAL_GENERALIZATION`; copy output establishes train capacity but not held-out retrieval generalization. |
| Stage 64 | Two-block pointer-generator capacity audit | Complete across five seeds. Decision: `TWO_BLOCK_POINTER_GENERATOR_CAPACITY_WITHOUT_RETRIEVAL_GENERALIZATION`; learned copy/vocab mixture preserves capacity but not retrieval generalization. |
| Stage 65 | Pointer-generator length-curriculum audit | Complete across five seeds. Decision: `POINTER_GENERATOR_LENGTH_CURRICULUM_WITHOUT_RETRIEVAL_GENERALIZATION`; length-40 curriculum preserves capacity and improves tiny text-fact QA but not retrieval generalization. |
| Stage 66 | Positional-copy expert audit | Complete across five seeds. Decision: `POSITIONAL_COPY_EXPERT_WITHOUT_RETRIEVAL_GENERALIZATION`; direct method-bias copy expert preserves capacity but not retrieval generalization. |
| Stage 67 | Content-key retrieval audit | Complete across five seeds. Decision: `CONTENT_KEY_RETRIEVAL_SOLVABLE_FOR_ALL_METHODS_NOT_PROMOTION`; visible content-key retrieval solves for all methods, including `no_position`. |
| Stage 68 | Content-key auxiliary transfer audit | Complete across five seeds. Decision: `CONTENT_KEY_AUXILIARY_TRANSFER_WITHOUT_RETRIEVAL_GENERALIZATION`; content-key auxiliary training preserves capacity but does not transfer to original retrieval generalization. |
| Stage 69 | Original multitask pointer-generator audit | Complete across five seeds. Decision: `ORIGINAL_MULTITASK_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`; original-task multitask training preserves capacity but not retrieval generalization. |
| Stage 70 | Strongest honest claim synthesis | Complete. Decision: `BOUND_STRONGEST_HONEST_CLAIM_WITH_RETRIEVAL_FAILURES`; current fair evidence supports a bounded compact/auditable claim, not RoPE replacement. |
| Stage 71 | Positional-bias copy upper-bound audit | Complete. Decision: `POSITIONAL_BIAS_COPY_PARTIAL_ORIGINAL_RETRIEVAL_UPPER_BOUND`; direct positional copy solves exact-offset for `rope_relative`, but not phase-cued retrieval. |
| Stage 72 | Phase-cued bias tie-support audit | Complete. Decision: `PHASE_CUED_TARGET_NOT_IN_PHASEWRAP_MAX_BIAS_SUPPORT`; PhaseWrap max-bias support misses the held-out phase-cued target. |
| Stage 73 | Phase-cued period-pair support audit | Complete. Decision: `PHASE_CUED_PERIOD_PAIR_SWEEP_DOES_NOT_REPAIR_SUPPORT`; every predeclared fixed period pair misses the held-out phase-cued target in max-score support. |
| Stage 74 | Leave-one-seed query-support audit | Complete. Decision: `LEAVE_ONE_SEED_QUERY_SUPPORT_SOLVES_PHASE_CUED_NOT_PROMOTION`; visible support recovery solves phase-cued retrieval for `no_position` too. |
| Stage 75 | Learned query-support head audit | Complete. Decision: `LEARNED_QUERY_SUPPORT_HEAD_SOLVES_PHASE_CUED_NOT_PROMOTION`; learned visible-cue recovery solves phase-cued retrieval for `no_position` too. |
| Stage 76 | Integrated support copy-head audit | Complete. Decision: `INTEGRATED_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; exact-offset is partially repaired, but phase-cued retrieval returns to zero for every method. |
| Stage 77 | Auxiliary support copy-head audit | Complete. Decision: `AUXILIARY_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; support supervision still does not preserve phase-cued held-out retrieval. |
| Stage 78 | Support coverage split audit | Complete. Decision: `SAME_SEED_SUPPORT_COVERAGE_SPLIT_EXPLAINS_STAGE77_FAILURE`; default same-seed train support has zero held-out phase-cued coverage. |
| Stage 79 | Support-complete auxiliary copy-head audit | Complete. Decision: `SUPPORT_COMPLETE_AUXILIARY_COPY_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support labels recover but phase-cued token retrieval still fails. |
| Stage 80 | Support-routed token selector audit | Complete. Decision: `SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; support-to-token routing repairs phase-cued retrieval for every method, including `no_position`. |
| Stage 81 | Soft support-routed token selector audit | Complete. Decision: `SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; learned support probabilities preserve the phase-cued repair, but every method solves, including `no_position`. |
| Stage 82 | Learned support-routing head audit | Complete. Decision: `LEARNED_SUPPORT_ROUTING_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support labels recover, but compact learned routing does not repair phase-cued retrieval. |
