...
| Paper referenced verifier/evidence artifacts that were not staged for public review. | The public evidence bundle is prepared for publication: `src/qrope/automated_stage_gates.py`, `scripts/verify_stage4_hardware_packet.py`, `scripts/verify_stage4_hardware_sweep.py`, Stage 4 JSON packet files under `logs/automated_stage_gates/stage4_hardware_packet/`, and a sweep manifest that passes for the active IBM Fez and Braket/Rigetti records while listing additional IBM/CX targets as deferred and IonQ as an excluded unavailable target. |
