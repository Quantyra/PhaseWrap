...
- Stage 6 through Stage 10 and Stage 12 through Stage 74 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, value-output capacity/hardening, compact QA, compact attention-bridge, period-pair audit, matched retrieval-bridge, full-context retrieval-attention, full-context feature-bridge, compact sequence/pointer-generator diagnostics, plateau audits, matched one-block decoder gates, capacity/generalization/stability audits, copy-output repair audits, learned pointer-generator audits, decoder-path plateau audits, two-block feasibility/hardening/attention-supervision audits, row-metadata upper bounds, standard-input cue diagnostics, support-prior diagnostics, pooled, seed-local, and leave-one-seed lookup diagnostics, fallback strictness diagnostics, support-complete learned decoder diagnostics, length-curriculum diagnostics, positional-copy expert diagnostics, content-key row-redesign, auxiliary-transfer, multitask diagnostics, strongest-claim synthesis, positional-bias copy upper bounds, tie-support audits, period-pair support audits, and visible query-support lookup diagnostics, or very small transformer experiments, not production transformer evaluations.
