...
- Stage 6 through Stage 10 and Stage 12 through Stage 52 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, value-output capacity/hardening, compact QA, compact attention-bridge, period-pair audit, matched retrieval-bridge, full-context retrieval-attention, full-context feature-bridge, compact sequence/pointer-generator diagnostics, plateau audits, matched one-block decoder gates, capacity/generalization/stability audits, copy-output repair audits, learned pointer-generator audits, decoder-path plateau audits, two-block feasibility audits, or very small transformer experiments, not production transformer evaluations.
...
| 2 | Stronger standard retrieval tasks | Stage 52 shows a stronger two-block residual decoder can fit train rows and tiny text-fact QA but still fails retrieval. Next, scale or redesign that stronger decoder path specifically for retrieval generalization, then rerun the same fair comparison rather than broadening claims through more compact copy-path diagnostics. |
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 52 now test adapters, period-pair alternatives, matched feature-budget bridges, full-context retrieval attention, nonlinear full-context features, post-hoc calibration, value-output diagnostics, copy-style output, trainable pointer-generators, generator-target hardening, compact and decoder-path plateau audits, a matched one-block decoder gate, capacity hardening, Adam generalization, five-seed Adam stability, fixed copy-decoder repair, learned pointer-generator repair, and two-block decoder feasibility. The behavior remains mixed: PhaseWrap ranking can recover under copy-aware output, but RoPE-like/no-position baselines lead the five-seed tiny text-fact QA lane, the fixed-copy exact-offset repair is RoPE-relative-led, learned pointer-generation does not preserve that repair, and the two-block feasibility audit still fails retrieval. |
