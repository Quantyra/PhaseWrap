...
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 19 now test adapters around PhaseWrap features and show that explicit distance terms are needed on exact-offset retrieval and value readout. Stages 17 and 18 exposed a value-output bottleneck; Stage 19 shows it can be improved under teacher-forced attention, so the next step is reintroducing learned positional attention. |
