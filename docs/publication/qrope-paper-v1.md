...
- Stage 6, Stage 7, Stage 8, Stage 9, Stage 10, Stage 12, Stage 13, Stage 14, Stage 15, Stage 16, and Stage 17 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, or very small transformer experiments, not production transformer evaluations.
...
| 3 | Comparable PhaseWrap mechanism | Stage 13 through Stage 17 now test adapters around PhaseWrap features and show that explicit distance terms are needed on exact-offset retrieval and value readout. Stage 17 also shows the learned value-output bottleneck needs optimization/capacity hardening before stronger transformer claims. |
