...
The strongest supported interpretation is that PhaseWrap-RoPE is a compact, auditable phase-wrap positional scoring rule whose cross-band signal can be preserved in recorded small-circuit hardware readouts and in selected toy downstream diagnostics. Stage 70 synthesizes the current fair comparisons as bounded evidence with retrieval failures preserved, and Stage 71 adds that direct positional-bias copy still does not solve the original phase-cued retrieval lane. The current evidence does not establish a replacement for RoPE, a production language-model result, or a broad hardware generalization claim.
...
- Stage 6 through Stage 10 and Stage 12 through Stage 71 are toy downstream, retrieval, compact trained positional-attention, positional-adapter, attention-readout, learned attention-readout, stability, value-output capacity/hardening, compact QA, compact attention-bridge, period-pair audit, matched retrieval-bridge, full-context retrieval-attention, full-context feature-bridge, compact sequence/pointer-generator diagnostics, plateau audits, matched one-block decoder gates, capacity/generalization/stability audits, copy-output repair audits, learned pointer-generator audits, decoder-path plateau audits, two-block feasibility/hardening/attention-supervision audits, row-metadata upper bounds, standard-input cue diagnostics, support-prior diagnostics, pooled and seed-local lookup diagnostics, fallback strictness diagnostics, support-complete learned decoder diagnostics, length-curriculum diagnostics, positional-copy expert diagnostics, content-key row-redesign, auxiliary-transfer, multitask diagnostics, strongest-claim synthesis, and positional-bias copy upper bounds, or very small transformer experiments, not production transformer evaluations.
