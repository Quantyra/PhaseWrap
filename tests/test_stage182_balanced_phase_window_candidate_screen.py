...
from qrope.stage182_balanced_phase_window_candidate_screen import (
    DESIGN_FAMILY_ID,
    run_stage182_balanced_phase_window_candidate_screen,
    write_stage182_outputs,
)
...
def test_stage182_runs_balanced_candidate_screen_without_hardware(tmp_path) -> None:
    ...
    result = run_stage182_balanced_phase_window_candidate_screen(
    ...
    assert result["decision"] == "BALANCED_PHASE_WINDOW_CANDIDATE_DOES_NOT_SUPPORT_HARDWARE_REOPEN"
...
def test_stage182_blocks_when_redesign_plan_not_ready(tmp_path) -> None:
    stage177 = tmp_path / "stage177.json"
    stage181 = tmp_path / "stage181.json"
    _stage177(stage177)
    ...
    assert result["decision"] == "BALANCED_PHASE_WINDOW_CANDIDATE_SCREEN_INCOMPLETE"
    assert "stage181_redesign_plan_not_ready" in result["blockers"]


def test_stage182_outputs_do_not_record_secrets_or_live_submit(tmp_path) -> None:
    stage177 = tmp_path / "stage177.json"
    stage181 = tmp_path / "stage181.json"
    _stage177(stage177)
    _stage181(stage181)
    ...
    paths = write_stage182_outputs(result, tmp_path / "out")
