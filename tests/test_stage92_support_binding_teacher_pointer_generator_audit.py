...
from qrope.stage92_support_binding_teacher_pointer_generator_audit import (
    build_blocked_result,
    run_stage92_audit,
    write_stage92_outputs,
)


def test_stage92_blocked_result_is_machine_readable() -> None:
    result = build_blocked_result()
    assert result["stage"] == "stage92_support_binding_teacher_pointer_generator_audit"
    ...
    assert result["source_stage"] == "stage91_curriculum_teacher_distilled_pointer_generator_audit"


def test_stage92_smoke_reports_three_block_distilled_pointer_generator_decision_or_blocked() -> None:
    result = run_stage92_audit(
        seeds=(307,),
    ...
    assert result["decision"]["decision"] in {
        "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED",
        "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED",
        "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_PARTIAL_RETRIEVAL",
        "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_ATTENTION_REPAIRED_TOKEN_FAILED",
        "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION",
    }
    assert result["source_stage"] == "stage91_curriculum_teacher_distilled_pointer_generator_audit"
    assert result["support_coverage"]["307"]["test_known_fraction"] == 1.0
    assert result["model"]["type"] == "three_block_pointer_generator_support_binding_teacher_decoder"
    assert "target-position attention auxiliary loss" in result["model"]["target_attention_supervision_policy"]
    assert "support-weighted copied-token binding loss" in result["model"]["support_binding_policy"]
    ...
    assert first_run["support_binding_weight"] == result["support_binding_weight"]
...
def test_stage92_outputs_are_written(tmp_path) -> None:
    result = run_stage92_audit(
        seeds=(307,),
    ...
    paths = write_stage92_outputs(result, tmp_path)
    ...
    assert manifest["stage"] == "stage92_support_binding_teacher_pointer_generator_audit"
    assert saved["stage"] == "stage92_support_binding_teacher_pointer_generator_audit"
