...
from qrope.stage17_small_decoder_value_model import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run deterministic Stage 17 small decoder value-model benchmark.")
    ...
    result = run_stage17_benchmark(
        seeds=args.seeds,
        context_lengths=args.context_lengths,
        examples_per_task_length=args.examples_per_task_length,
        epochs=args.epochs,
    )
    paths = write_stage17_outputs(result, args.output_dir)
    print_stage17_table(result)
