...
PUBLIC_TEXT_FILES = [
    Path("README.md"),
    Path("REVIEWER_START.md"),
    Path("PATENTS.md"),
    Path("NOTICE"),
    Path("CITATION.cff"),
    Path(".zenodo.json"),
    Path("Dockerfile.reviewer"),
    Path(".dockerignore"),
    Path(".pre-commit-config.yaml"),
    Path("pyrightconfig.json"),
    Path("docs/publication/quickstart-results-summary-v1.md"),
    Path("docs/publication/phasewrap-research-program-decision-v1.md"),
    ...
    Path("docs/index.md"),
    Path("docs/stage-to-claim-map.md"),
...
REQUIRED_FILES = [
    *PUBLIC_TEXT_FILES,
    ...
    Path("src/qrope/cli.py"),
    Path("src/qrope/scoring.py"),
    Path("src/qrope/stage219_rope_substitution_gate.py"),
    Path("src/qrope/verify/__init__.py"),
    Path("src/qrope/verify/publication.py"),
    Path("src/qrope/verify_publication.py"),
    Path("src/qrope/paths.py"),
    Path("src/phasewrap/__init__.py"),
    Path("tests/test_scoring_api.py"),
    Path("tests/test_env_utils.py"),
    Path("tests/test_phasewrap_alias.py"),
    Path("tests/test_publication_verify_cli.py"),
...
REQUIRED_PUBLIC_STRINGS = [
    ...
    "The positive replacement research line is closed",
    "support-routing repairs solve phase-cued retrieval for all tested methods, including `no_position`",
    ...
    "low-prominence legal notice",
    "low-prominence legal mention",
    "not part of the scientific claim",
    "Patent prosecution and conversion strategy is outside this repository",
    "PROGRAM_DECISION_NEGATIVE_RESULTS_FIRST",
    "The best course of action is to stop treating PhaseWrap as an active positive RoPE-replacement or quantum-advantage research program",
    "Write one essential methodology paper first",
    "New hardware spending requires a preregistered matched-encoding protocol",
    "METHODOLOGY_PAPER_DRAFT_V1",
    "Assistance pipelines can repair retrieval tasks without isolating the positional mechanism under test",
    "Stage 216-218 hardware evidence is real but bounded",
    ...
    "DECISION_MEMO_EXECUTED_GITHUB_RELEASE_PUBLIC",
