...
STAGE34_SCHEMA_VERSION = "qrope_stage34_small_decoder_value_bridge_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage34_small_decoder_value_bridge"
DEFAULT_MODEL_SEEDS = (3407, 3413, 3433, 3449, 3457)
METHOD_NAMES = (
    "no_position",
    "alibi",
    "rope_relative",
...
def decoder_value_bridge_features(row: Stage14Example, method_name: str) -> np.ndarray:
    candidate_deltas = row.query_pos - np.array(row.key_positions, dtype=float)
    recency = -candidate_deltas / float(row.query_pos)
    diff = float(row.reference_delta) - candidate_deltas
    features = np.zeros((len(row.key_positions), FEATURE_DIM), dtype=float)
    features[:, 0] = 1.0
    features[:, 1] = recency
    features[:, 2] = np.array(row.candidate_values, dtype=float) / float(VALUE_VOCAB_SIZE)

    if method_name == "no_position":
        return features
    if method_name == "alibi":
        features[:, 3] = recency
        features[:, 4] = recency * recency
        return features
    if method_name == "rope_relative":
        ...
        features[:, 6] = features[:, 3] * recency
        features[:, 7] = features[:, 4] * recency
        return features
    ...
    features[:, 8] = phase * recency
    features[:, 9] = features[:, 7] * recency
...
def _init_params(*, method_name: str, model_seed: int, hidden_dim: int, value_embed_dim: int) -> dict[str, np.ndarray]:
    seed_text = f"stage34:{method_name}:{model_seed}:{FEATURE_DIM}:{hidden_dim}:{value_embed_dim}"
...
def _loss_and_gradient(rows: list[Stage14Example], method_name: str, params: dict[str, np.ndarray], l2: float) -> tuple[float, dict[str, np.ndarray]]:
    ...
    for key, value in params.items():
        grads[key] *= scale
        if key != "output_b" and key != "b1" and key != "b2":
...
def train_decoder_value_bridge(
...
def evaluate_decoder_value_bridge(rows: list[Stage14Example], method_name: str, params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
...
def _aggregate_runs(run_rows: list[dict[str, Any]], *, method_name: str) -> dict[str, Any]:
    ...
    for metric_name in metric_names:
        ...
        ci = _metric_ci(values, seed_text=f"stage34:{method_name}:{metric_name}")
    ...
    *,
    ...
    for method_name in method_names:
        for model_seed in model_seeds:
            training = train_decoder_value_bridge(splits["train"], method_name, model_seed=model_seed, epochs=epochs)
            training_records.append(training)
            params = _params_from_record(training)
            row = evaluate_decoder_value_bridge(splits["test"], method_name, params)
    ...
    return {
        "schema_version": STAGE34_SCHEMA_VERSION,
        "stage": "stage34_small_decoder_value_bridge",
        ...
        "model": {
            "type": "nonlinear_attention_plus_learned_value_output_bridge",
            "feature_dim": FEATURE_DIM,
            "hidden_dim": HIDDEN_DIM,
        ...
        "task": {
            "description": "Train-short/test-long key-value retrieval with learned attention and learned value-token output.",
            "target_construction": "Targets are explicit Stage 12 retrieval-rule value tokens, not PhaseWrap-selected labels.",
            "scope": "This is a compact decoder-style value bridge, not a production decoder-only language-model benchmark.",
        },
        "claim_boundary": {
            "supported": [
                "A deterministic multi-initialization compact decoder-style value benchmark on non-phase-cued retrieval rows.",
                "A stronger follow-up to Stage 32/33 because the retrieved value must pass through learned value embeddings and an output projection.",
...
def write_stage34_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage34_table(result: dict[str, Any]) -> None:
