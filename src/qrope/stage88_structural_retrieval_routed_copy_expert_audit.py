...
from .stage10_small_decoder_transformer import TASK_NAMES, TEST_LENGTHS, TRAIN_LENGTHS, VALIDATION_LENGTHS, VOCAB_SIZE, Stage10Example, _expected_calibration_error, autograd_available, make_stage10_splits, positional_bias
...
STAGE88_SCHEMA_VERSION = "qrope_stage88_structural_retrieval_routed_copy_expert_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage88_structural_retrieval_routed_copy_expert_audit"
DEFAULT_SUPPORT_ROUTE_WEIGHT = 1.0
DEFAULT_EXACT_OFFSET_ROUTE_WEIGHT = 1.0
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            ...
            "Evidence about whether Stage87 phase-cued support routing and Stage71 exact-offset positional-bias copy can jointly solve original held-out retrieval.",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE88_SCHEMA_VERSION,
            "stage": "stage88_structural_retrieval_routed_copy_expert_audit",
            "source_stage": "stage87_in_decoder_support_routed_copy_expert_audit",
            ...
            "exact_offset_route_weight": DEFAULT_EXACT_OFFSET_ROUTE_WEIGHT,
...
def _positional_bias_copy_probabilities(row: Stage10Example, method_name: str) -> np.ndarray:
...
def _mixed_probabilities(
    ...
    support_route_weight: float,
    exact_offset_route_weight: float,
    ...
    elif row.task == "exact_offset_passkey":
        routed_probs = _positional_bias_copy_probabilities(row, method_name)
        probabilities = exact_offset_route_weight * routed_probs + (1.0 - exact_offset_route_weight) * decoder_probs
    else:
...
def _predict(
    ...
    support_route_weight: float,
    exact_offset_route_weight: float,
) -> tuple[float, int, float, float, int | None, float]:
    probabilities, copy_gate, support_probabilities, target_attention = _mixed_probabilities(
        vector,
        row,
        method_name,
        support_classes,
        support_route_weight,
        exact_offset_route_weight,
    )
...
def evaluate_structural_retrieval_routed_copy_expert(
    ...
    *,
    ...
    exact_offset_route_weight: float = DEFAULT_EXACT_OFFSET_ROUTE_WEIGHT,
) -> dict[str, float]:
    losses: list[float] = []
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "STRUCTURAL_RETRIEVAL_ROUTED_COPY_EXPERT_CAPACITY_NOT_ESTABLISHED"
        boundary = "The structural retrieval-routed copy expert path does not establish train capacity."
    elif len(retrieval_solved) == len(RETRIEVAL_TASKS):
        phasewrap_led = all(retrieval_best[task]["method"].startswith("phasewrap") for task in RETRIEVAL_TASKS)
        if phasewrap_led:
            decision = "STRUCTURAL_RETRIEVAL_ROUTED_COPY_EXPERT_SOLVES_RETRIEVAL_REVIEW_REQUIRED"
            boundary = "Structural retrieval-routed copy experts solve both retrieval lanes with PhaseWrap-led methods; review required before any claim update because the mechanism is structural."
        else:
            decision = "STRUCTURAL_RETRIEVAL_ROUTED_COPY_EXPERT_SOLVES_RETRIEVAL_NOT_PROMOTION"
            boundary = "Structural retrieval-routed copy experts solve both retrieval lanes, but the result is method-nonspecific or not PhaseWrap-led and is not positional-method promotion evidence."
    elif retrieval_solved:
        decision = "STRUCTURAL_RETRIEVAL_ROUTED_COPY_EXPERT_PARTIAL_RETRIEVAL"
        boundary = "Structural retrieval-routed copy experts repair at least one retrieval lane but not the full retrieval gate."
    else:
        decision = "STRUCTURAL_RETRIEVAL_ROUTED_COPY_EXPERT_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "Structural retrieval-routed copy experts still do not repair held-out retrieval."
    return {
        **base,
        "decision": decision,
...
def run_stage88_audit(
    *,
            ...
            try:
                ...
                for task_name in TASK_NAMES:
                    ...
                    row: dict[str, Any] = {
                        ...
                        "exact_offset_route_weight": exact_offset_route_weight,
                    ...
                    for split_name, split_rows in (("train", task_train_rows), ("validation", validation_rows), ("test", test_rows)):
                        metrics = evaluate_structural_retrieval_routed_copy_expert(
                            split_rows,
                            stage10_method,
                            trained["weights"],
                            support_classes,
    ...
    result = {
        ...
        "stage": "stage88_structural_retrieval_routed_copy_expert_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_support_complete_rows_multitask_structural_retrieval_routed_copy_expert_v1",
        ...
        "model": {
            "type": "two_block_pointer_generator_structural_retrieval_routed_copy_expert",
            "optimizer": "full_batch_adam_dual_auxiliary",
            "value_output_mode": "learned pointer-generator distribution mixed with structural copied-token experts for phase-cued and exact-offset rows",
            "phase_cued_routing_policy": "evaluation uses decoder-predicted support probabilities, not gold support labels, target_pos, target_delta, or reference_delta",
            "exact_offset_routing_policy": "evaluation copies the argmax prefix token under each method's own positional-bias score",
            "promotion_limit": "the retrieval routing rules are structural experts and not standard free decoder-only language modeling",
...
def write_stage88_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage88_summary(result: dict[str, Any]) -> None:
