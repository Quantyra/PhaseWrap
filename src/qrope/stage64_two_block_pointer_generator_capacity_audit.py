...
STAGE64_SCHEMA_VERSION = "qrope_stage64_two_block_pointer_generator_capacity_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage64_two_block_pointer_generator_capacity_audit"


def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            "A no-credential two-block learned pointer-generator capacity audit over the support-complete row family.",
            "Evidence about whether a learned copy/vocab mixture preserves the Stage 63 copy-output capacity repair.",
        ...
        "excluded": [
            ...
            "a claim that learned pointer-generation is equivalent to full decoder-only language-model validation",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE64_SCHEMA_VERSION,
            "stage": "stage64_two_block_pointer_generator_capacity_audit",
            "source_stage": "stage63_two_block_copy_output_capacity_audit",
...
def _init_vector(seed: int) -> np.ndarray:
    ...
    size = VOCAB_SIZE * MODEL_DIM + 8 * matrix_size + MODEL_DIM * VOCAB_SIZE + 2
...
def _unpack(vector: Any):
    ...
    return emb, matrices, output, vector[index], vector[index + 1]
...
def _row_probabilities(vector: Any, row: Stage10Example, method_name: str):
    ...
    emb, matrices, output, pos_scale, copy_logit = _unpack(vector)
    ...
    return copy_gate * copy_probabilities + (1.0 - copy_gate) * generator_probabilities, copy_gate
...
def train_two_block_pointer_generator_decoder(
    ...
    *,
    ...
    _, _, _, pos_scale, copy_logit = _unpack(vector)
    return {
        "weights": vector,
...
def _predict(vector: Any, row: Stage10Example, method_name: str) -> tuple[float, int, float, float]:
    ...
    return float(probs[row.label_token]), int(sorted_indices.index(row.label_token) + 1), float(probs[sorted_indices[0]]), float(anp.asarray(copy_gate))


def evaluate_two_block_pointer_generator_decoder(rows: list[Stage10Example], method_name: str, vector: Any) -> dict[str, float]:
    losses: list[float] = []
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "TWO_BLOCK_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED"
        boundary = "The learned copy/vocab mixture does not preserve the Stage 63 copy-output capacity repair."
    elif all(value >= GENERALIZATION_TOP1_THRESHOLD for value in base["retrieval_best_top1"].values()):
        decision = "TWO_BLOCK_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED"
        boundary = "The learned copy/vocab mixture generalizes retrieval; review method ordering before any claim update."
    else:
        decision = "TWO_BLOCK_POINTER_GENERATOR_CAPACITY_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "The learned copy/vocab mixture establishes train capacity but does not generalize all retrieval lanes."
    return {
        **base,
        "decision": decision,
...
def _add_copy_gate_aggregates(aggregate_table: list[dict[str, Any]], run_table: list[dict[str, Any]]) -> list[dict[str, Any]]:
    for aggregate_row in aggregate_table:
        rows = [
            row
            for row in run_table
            if row["task"] == aggregate_row["task"] and row["method"] == aggregate_row["method"]
        ]
        if not rows:
            continue
        aggregate_row["learned_copy_gate_mean"] = round(float(np.mean([row["learned_copy_gate"] for row in rows])), 6)
        for split_name in ("train", "validation", "test"):
            aggregate_row[f"{split_name}_mean_copy_gate_mean"] = round(
                float(np.mean([row[f"{split_name}_mean_copy_gate"] for row in rows])),
                6,
            )
    return aggregate_table


def run_stage64_audit(
    *,
    ...
    result = {
        ...
        "model": {
            "type": "support_complete_two_block_learned_pointer_generator_decoder",
            "model_dim": MODEL_DIM,
...
def write_stage64_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage64_summary(result: dict[str, Any]) -> None:
