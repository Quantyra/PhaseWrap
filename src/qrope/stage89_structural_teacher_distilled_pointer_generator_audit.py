...
STAGE89_SCHEMA_VERSION = "qrope_stage89_structural_teacher_distilled_pointer_generator_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage89_structural_teacher_distilled_pointer_generator_audit"
DEFAULT_EPOCHS = 10
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            ...
            "Evidence about whether Stage88 structural retrieval routes can be distilled into a free learned decoder evaluated without structural copy routing.",
        ...
        "excluded": [
            ...
            "a claim that structural teacher distillation is standard free decoder-only language modeling",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE89_SCHEMA_VERSION,
            "stage": "stage89_structural_teacher_distilled_pointer_generator_audit",
...
def _init_dual_aux_vector(seed: int, support_class_count: int) -> np.ndarray:
...
def _unpack_dual_aux(vector: Any, support_class_count: int):
...
def _row_outputs(vector: Any, row: Stage10Example, method_name: str, support_class_count: int):
    ...
    emb, matrices, output, pos_scale, copy_logit, support_output = _unpack_dual_aux(vector, support_class_count)
    ...
    copy_probabilities = anp.dot(attention_2, _copy_indicator(row))
...
def _row_loss(
    ...
    probabilities, _, support_probabilities, attention_1, attention_2 = _row_outputs(vector, row, method_name, len(support_classes))
    ...
    if row.task in RETRIEVAL_TASKS:
        ...
        target_attention_loss = -0.5 * (anp.log(attention_1[target_index] + 1e-12) + anp.log(attention_2[target_index] + 1e-12))
...
def train_structural_teacher_distilled_pointer_generator_decoder(
    rows: list[Stage10Example],
    method_name: str,
    *,
    seed: int,
    ...
    vector = _init_dual_aux_vector(seed, len(support_classes))
    gradient = grad(
    ...
    _, _, _, pos_scale, copy_logit, _ = _unpack_dual_aux(vector, len(support_classes))
    return {
        "weights": vector,
        "optimizer": "full_batch_adam_structural_teacher_distillation",
        "epochs": epochs,
...
def _predict(vector: Any, row: Stage10Example, method_name: str, support_classes: tuple[int, ...]) -> tuple[float, int, float, float, int | None, float]:
    probabilities, copy_gate, support_probabilities, attention_1, attention_2 = _row_outputs(vector, row, method_name, len(support_classes))
...
def evaluate_structural_teacher_distilled_pointer_generator(rows: list[Stage10Example], method_name: str, vector: Any, support_classes: tuple[int, ...]) -> dict[str, float]:
    losses: list[float] = []
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "STRUCTURAL_TEACHER_DISTILLED_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED"
        boundary = "Structural teacher distillation does not establish train capacity."
    elif all(value >= GENERALIZATION_TOP1_THRESHOLD for value in retrieval_best_top1.values()):
        decision = "STRUCTURAL_TEACHER_DISTILLED_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED"
        boundary = "Training-time structural teacher distillation generalizes all retrieval lanes without evaluation-time structural routing; review method ordering before any claim update."
    elif generalized_retrieval:
        decision = "STRUCTURAL_TEACHER_DISTILLED_POINTER_GENERATOR_PARTIAL_RETRIEVAL"
        boundary = "Training-time structural teacher distillation generalizes at least one retrieval lane but not the full retrieval gate."
    elif attention_repaired:
        decision = "STRUCTURAL_TEACHER_DISTILLED_POINTER_GENERATOR_ATTENTION_REPAIRED_TOKEN_FAILED"
        boundary = "Structural teacher distillation repairs target attention for at least one retrieval lane, but free token retrieval remains below threshold."
    else:
        decision = "STRUCTURAL_TEACHER_DISTILLED_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "Structural teacher distillation preserves capacity but does not repair free held-out retrieval generalization."
    return {
        **base,
        "decision": decision,
...
def run_stage89_audit(
    *,
    ...
    for seed in seeds:
        ...
        for method_name in method_names:
            try:
                ...
                trained = train_structural_teacher_distilled_pointer_generator_decoder(
                ...
                for task_name in TASK_NAMES:
                    ...
                    for split_name, split_rows in (("train", task_train_rows), ("validation", validation_rows), ("test", test_rows)):
                        metrics = evaluate_structural_teacher_distilled_pointer_generator(split_rows, stage10_method, trained["weights"], support_classes)
    ...
    result = {
        ...
        "stage": "stage89_structural_teacher_distilled_pointer_generator_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_support_complete_rows_multitask_structural_teacher_distilled_pointer_generator_v1",
        ...
        "model": {
            "type": "two_block_pointer_generator_structural_teacher_distilled_decoder",
            "optimizer": "full_batch_adam_structural_teacher_distillation",
...
def write_stage89_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage89_summary(result: dict[str, Any]) -> None:
