...
from .stage13_positional_adapter import METHOD_NAMES, positional_features, split_examples


STAGE28_SCHEMA_VERSION = "qrope_stage28_ruler_attention_bridge_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage28_ruler_attention_bridge"
DEFAULT_MODEL_SEEDS = (2801, 2819, 2833, 2837, 2843)
HIDDEN_DIM = 8
...
def _init_params(feature_dim: int, *, method_name: str, model_seed: int, hidden_dim: int) -> dict[str, np.ndarray]:
    seed_text = f"stage28:{method_name}:{model_seed}:{feature_dim}:{hidden_dim}"
...
def _forward(row: Stage12Example, method_name: str, params: dict[str, np.ndarray]) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    ...
    return probabilities, {"features": features, "hidden": hidden}
...
def train_attention_bridge(
    ...
    *,
    ...
    param_bytes = json.dumps({key: np.round(value, 8).tolist() for key, value in params.items()}, sort_keys=True).encode("utf-8")
    return {
        "method": method_name,
...
def evaluate_attention_bridge(rows: list[Stage12Example], method_name: str, params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
...
def _aggregate_runs(run_rows: list[dict[str, Any]], *, method_name: str) -> dict[str, Any]:
    ...
    for metric_name in metric_names:
        ...
        ci = _metric_ci(values, seed_text=f"stage28:{method_name}:{metric_name}")
...
def run_stage28_benchmark(
    *,
    ...
    for method_name in method_names:
        for model_seed in model_seeds:
            ...
            for task_name in TASK_NAMES:
                ...
                task_result = evaluate_attention_bridge(task_rows, method_name, params)
    ...
    return {
        ...
        "stage": "stage28_ruler_attention_bridge",
        ...
        "model": {
            "type": "one_hidden_layer_attention_bridge_over_stage12_positional_features",
            "hidden_dim": HIDDEN_DIM,
            "epochs": epochs,
        ...
        "task": {
            ...
            "scope": "This is a compact attention bridge over standard retrieval-style rows, not a full decoder-only language-model benchmark.",
        },
        "claim_boundary": {
            "supported": [
                "A deterministic multi-initialization attention-bridge comparison on non-phase-cued RULER-style retrieval rows.",
                "Evidence about whether learned nonlinear attention over PhaseWrap-derived features is competitive on this compact retrieval bridge.",
...
def write_stage28_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage28_table(result: dict[str, Any]) -> None:
    columns = (
        "method",
        "run_count",
        "top1_accuracy_mean",
