...
STAGE32_SCHEMA_VERSION = "qrope_stage32_full_context_feature_bridge_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage32_full_context_feature_bridge"
DEFAULT_MODEL_SEEDS = (3203, 3217, 3221, 3229, 3251)
METHOD_NAMES = (
    "no_position",
    "alibi",
    "rope_relative",
...
def full_context_bridge_features(row: Stage12Example, method_name: str) -> np.ndarray:
    ...
    features[:, 7] = -np.abs(diff) / float(row.query_pos)
    features[:, 8] = content * phase
    features[:, 9] = content * features[:, 7]
    ...
    if method_name == "phasewrap_multiscale_adapter":
        ...
        features[:, 10] = alt_8_24
        features[:, 11] = alt_9_15
        return features
...
def _init_params(*, method_name: str, model_seed: int, hidden_dim: int) -> dict[str, np.ndarray]:
    seed_text = f"stage32:{method_name}:{model_seed}:{FEATURE_DIM}:{hidden_dim}"
...
def _loss_and_gradient(rows: list[Stage12Example], method_name: str, params: dict[str, np.ndarray], l2: float) -> tuple[float, dict[str, np.ndarray]]:
...
def evaluate_full_context_bridge(rows: list[Stage12Example], method_name: str, params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
...
def _aggregate_runs(run_rows: list[dict[str, Any]], *, method_name: str) -> dict[str, Any]:
    ...
    for metric_name in metric_names:
        ...
        ci = _metric_ci(values, seed_text=f"stage32:{method_name}:{metric_name}")
    ...
    *,
    ...
    for method_name in method_names:
        for model_seed in model_seeds:
            ...
            row = evaluate_full_context_bridge(splits["test"], method_name, params)
            ...
            for task_name in TASK_NAMES:
                ...
                task_result = evaluate_full_context_bridge(task_rows, method_name, params)
    ...
    return {
        "schema_version": STAGE32_SCHEMA_VERSION,
        "stage": "stage32_full_context_feature_bridge",
        ...
        "model": {
            "type": "one_hidden_layer_full_context_feature_bridge",
            "feature_dim": FEATURE_DIM,
            "hidden_dim": HIDDEN_DIM,
            "parameter_count": FEATURE_DIM * HIDDEN_DIM + HIDDEN_DIM + HIDDEN_DIM + 1,
            "epochs": epochs,
        ...
        "task": {
            "description": "Full-prefix non-phase-cued retrieval over Stage 12 rows with a nonlinear feature bridge.",
            "target_construction": "Targets are selected by explicit retrieval rules, not by the PhaseWrap score.",
            "scope": "This is a full-context retrieval feature bridge, not a full decoder-only language-model benchmark.",
        },
        "claim_boundary": {
            "supported": [
                "A deterministic multi-initialization nonlinear full-context retrieval bridge on Stage 12 non-phase-cued rows.",
                "A direct follow-up to the Stage 31 simple-attention failure that tests richer PhaseWrap-derived features.",
...
def write_stage32_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage32_table(result: dict[str, Any]) -> None:
