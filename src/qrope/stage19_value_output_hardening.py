...
STAGE19_SCHEMA_VERSION = "qrope_stage19_value_output_hardening_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage19_value_output_hardening"
DEFAULT_CONFIGS: tuple[dict[str, Any], ...] = (
    {"config_name": "adam_embed12", "embed_dim": 12, "epochs": 500, "learning_rate": 0.03, "l2": 0.00001},
    {"config_name": "adam_embed32", "embed_dim": 32, "epochs": 500, "learning_rate": 0.03, "l2": 0.00001},
    {"config_name": "adam_embed64", "embed_dim": 64, "epochs": 500, "learning_rate": 0.03, "l2": 0.00001},
)
...
def _attention(row: Stage14Example) -> np.ndarray:
...
def _init_params(config_name: str, seed: int, embed_dim: int) -> dict[str, np.ndarray]:
    digest = hashlib.sha256(f"stage19:{config_name}:{seed}:{embed_dim}".encode("utf-8")).hexdigest()
...
def train_hardened_value_output(
    ...
    *,
    ...
    params = _init_params(config_name, seed, embed_dim)
...
def evaluate_hardened_value_output(rows: list[Stage14Example], params: dict[str, np.ndarray], *, config_name: str, split_name: str) -> dict[str, Any]:
    losses: list[float] = []
    ...
    top1_ci = _bootstrap_ci(top1_hits, seed_text=f"{config_name}:{split_name}:top1")
    mrr_ci = _bootstrap_ci(reciprocal_ranks, seed_text=f"{config_name}:{split_name}:mrr")
    prob_ci = _bootstrap_ci(target_masses, seed_text=f"{config_name}:{split_name}:prob")
    return {
        "config_name": config_name,
        "split": split_name,
        "row_count": len(rows),
...
def run_stage19_benchmark(
    *,
    ...
    for config in configs:
        training = train_hardened_value_output(splits["train"], **config)
        params = _params_from_record(training)
        training_records.append(training)
        for split_name in ("train", "validation", "test"):
            table.append(evaluate_hardened_value_output(splits[split_name], params, config_name=config["config_name"], split_name=split_name))
    test_rows = [row for row in table if row["split"] == "test"]
    selection_table = sorted(
        test_rows,
        key=lambda row: (row["top1_accuracy"], row["mrr"], row["mean_target_value_probability"], row["config_name"]),
        reverse=True,
    )
    return {
        "schema_version": STAGE19_SCHEMA_VERSION,
        "stage": "stage19_value_output_hardening",
        "dataset": "deterministic_value_output_hardening_probe_v1",
        "no_hardware_submission": True,
        "attention_mode": "teacher_forced_target_attention",
        "seeds": list(seeds),
        ...
        "configs": [{key: value for key, value in config.items()} for config in configs],
        "model": {
            "type": "teacher_forced_attention_value_output_hardening_probe",
        ...
        "claim_boundary": {
            "supported": [
                "A deterministic capacity and optimization hardening probe for the Stage 17 and Stage 18 learned value-output bottleneck.",
                "Evidence about whether larger value embeddings and Adam optimization improve teacher-forced value-token readout.",
            ],
            "excluded": [
        ...
        "best_config_by_test_top1_mrr": selection_table[0]["config_name"],
    }


def write_stage19_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage19_table(result: dict[str, Any]) -> None:
    columns = ("config_name", "split", "row_count", "top1_accuracy", "mrr", "mean_target_value_probability", "mean_first_relevant_value_rank")
    print(" | ".join(columns))
