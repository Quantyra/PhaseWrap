...
STAGE90_SCHEMA_VERSION = "qrope_stage90_three_block_teacher_distilled_pointer_generator_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage90_three_block_teacher_distilled_pointer_generator_audit"
DEFAULT_EPOCHS = 10
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            ...
            "Evidence about whether added learned decoder depth lets Stage88 structural retrieval routes distill into free evaluation without structural copy routing.",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE90_SCHEMA_VERSION,
            "stage": "stage90_three_block_teacher_distilled_pointer_generator_audit",
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "THREE_BLOCK_TEACHER_DISTILLED_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED"
        boundary = "Three-block structural teacher distillation does not establish train capacity."
    elif all(value >= GENERALIZATION_TOP1_THRESHOLD for value in retrieval_best_top1.values()):
        decision = "THREE_BLOCK_TEACHER_DISTILLED_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED"
        boundary = "Three-block training-time structural teacher distillation generalizes all retrieval lanes without evaluation-time structural routing; review method ordering before any claim update."
    elif generalized_retrieval:
        decision = "THREE_BLOCK_TEACHER_DISTILLED_POINTER_GENERATOR_PARTIAL_RETRIEVAL"
        boundary = "Three-block training-time structural teacher distillation generalizes at least one retrieval lane but not the full retrieval gate."
    elif attention_repaired:
        decision = "THREE_BLOCK_TEACHER_DISTILLED_POINTER_GENERATOR_ATTENTION_REPAIRED_TOKEN_FAILED"
        boundary = "Three-block structural teacher distillation repairs target attention for at least one retrieval lane, but free token retrieval remains below threshold."
    else:
        decision = "THREE_BLOCK_TEACHER_DISTILLED_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "Three-block structural teacher distillation preserves capacity but does not repair free held-out retrieval generalization."
    return {
        **base,
        "decision": decision,
...
def run_stage90_audit(
    *,
    ...
    result = {
        ...
        "stage": "stage90_three_block_teacher_distilled_pointer_generator_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_support_complete_rows_multitask_three_block_teacher_distilled_pointer_generator_v1",
        ...
        "model": {
            "type": "three_block_pointer_generator_structural_teacher_distilled_decoder",
            "optimizer": "full_batch_adam_three_block_structural_teacher_distillation",
...
def write_stage90_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage90_summary(result: dict[str, Any]) -> None:
