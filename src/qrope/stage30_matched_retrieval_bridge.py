...
STAGE30_SCHEMA_VERSION = "qrope_stage30_matched_retrieval_bridge_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage30_matched_retrieval_bridge"
DEFAULT_MODEL_SEEDS = (3001, 3011, 3019, 3023, 3037)
METHOD_NAMES = (
    "no_position",
    "alibi",
    "rope_relative",
...
def matched_positional_features(row: Stage12Example, method_name: str) -> np.ndarray:
    candidate_positions = np.array(row.key_positions, dtype=int)
    candidate_deltas = row.query_pos - candidate_positions.astype(float)
    tokens = np.array([row.tokens[position] for position in row.key_positions], dtype=int)
    ...
    features = np.zeros((len(candidate_positions), FEATURE_DIM), dtype=float)
    features[:, 0] = 1.0
    features[:, 1] = content
    features[:, 2] = recency
    ...
    if method_name == "alibi":
        features[:, 3] = recency
        features[:, 4] = content * recency
        return features
    if method_name == "rope_relative":
        inv_freq = _rope_inverse_frequencies()
        rope = np.cos(diff[:, None] * inv_freq[None, :])
        features[:, 3] = np.mean(rope, axis=1)
        features[:, 4] = np.max(rope, axis=1)
        features[:, 5] = np.min(rope, axis=1)
        features[:, 6] = content * features[:, 3]
        features[:, 7] = content * features[:, 4]
        return features
    if method_name == "sinusoidal":
        ...
        features[:, 3] = np.mean(waves, axis=1)
        features[:, 4] = np.max(waves, axis=1)
        features[:, 5] = np.min(waves, axis=1)
        features[:, 6] = content * features[:, 3]
        features[:, 7] = content * features[:, 4]
        return features

    phase_scores = np.array([_phasewrap_score(row.reference_delta, int(delta)) for delta in candidate_deltas], dtype=float)
    ...
    features[:, 3] = phase_scores
    features[:, 4] = cos8
    features[:, 5] = cos12
    features[:, 6] = cos8 * cos12
    features[:, 7] = content * phase_scores
    if method_name == "phasewrap_bias":
        return features
    if method_name == "phasewrap_residual_adapter":
        features[:, 8] = content * cos8
        features[:, 9] = content * cos12
        return features
    if method_name == "phasewrap_distance_adapter":
        features[:, 8] = content * cos8
        features[:, 9] = content * cos12
        features[:, 10] = -np.abs(diff) / float(row.query_pos)
        features[:, 11] = diff / float(row.query_pos)
        return features
...
def _target_distribution(row: Stage12Example) -> np.ndarray:
    ...
    for position in row.target_positions:
        targets[candidate_positions.index(position)] = 1.0 / float(len(row.target_positions))
    return targets


def _init_params(*, method_name: str, model_seed: int, hidden_dim: int) -> dict[str, np.ndarray]:
    seed_text = f"stage30:{method_name}:{model_seed}:{FEATURE_DIM}:{hidden_dim}"
...
def _forward(row: Stage12Example, method_name: str, params: dict[str, np.ndarray]) -> tuple[np.ndarray, dict[str, np.ndarray]]:
    features = matched_positional_features(row, method_name)
    hidden_pre = features @ params["w1"] + params["b1"]
    hidden = np.tanh(hidden_pre)
    logits = hidden @ params["w2"] + float(params["b2"][0])
    return _softmax(logits), {"features": features, "hidden": hidden}
...
def train_matched_bridge(
    ...
    *,
    ...
    return {
        ...
        "parameter_count": FEATURE_DIM * hidden_dim + hidden_dim + hidden_dim + 1,
        "epochs": epochs,
...
def _first_relevant_rank(probabilities: np.ndarray, target_indices: tuple[int, ...]) -> int:
...
def evaluate_matched_bridge(rows: list[Stage12Example], method_name: str, params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
...
def _aggregate_runs(run_rows: list[dict[str, Any]], *, method_name: str) -> dict[str, Any]:
    ...
    for metric_name in metric_names:
        ...
        ci = _metric_ci(values, seed_text=f"stage30:{method_name}:{metric_name}")
    ...
    *,
    ...
    for method_name in method_names:
        for model_seed in model_seeds:
            training = train_matched_bridge(splits["train"], method_name, model_seed=model_seed, epochs=epochs)
            training_records.append(training)
            params = _params_from_record(training)
            row = evaluate_matched_bridge(splits["test"], method_name, params)
            ...
            for task_name in TASK_NAMES:
                ...
                task_result = evaluate_matched_bridge(task_rows, method_name, params)
    ...
    slim_training_records = [
        {
            ...
            "l2": record["l2"],
            "param_sha256": record["param_sha256"],
            "training_history": record["training_history"],
            "final_training_loss": record["final_training_loss"],
        }
        for record in training_records
    ]
    return {
        "schema_version": STAGE30_SCHEMA_VERSION,
        "stage": "stage30_matched_retrieval_bridge",
        ...
        "model": {
            "type": "matched_feature_budget_one_hidden_layer_retrieval_attention_bridge",
            "feature_dim": FEATURE_DIM,
            "hidden_dim": HIDDEN_DIM,
            "parameter_count": FEATURE_DIM * HIDDEN_DIM + HIDDEN_DIM + HIDDEN_DIM + 1,
            "epochs": epochs,
            "trained_parameters": "fixed-width feature-to-hidden weights, hidden bias, hidden-to-attention-logit weights, logit bias",
        },
        "task": {
            "description": "Non-phase-cued passkey, multi-needle, and aggregation-style retrieval over Stage 12 rows.",
            "target_construction": "Targets are selected by explicit retrieval rules, not by the PhaseWrap score.",
            "scope": "This is a matched feature-budget retrieval bridge, not a full decoder-only language-model benchmark.",
        },
        "claim_boundary": {
            "supported": [
                "A deterministic multi-initialization retrieval-bridge comparison on non-phase-cued RULER-style rows.",
                "Every positional variant uses the same input feature width, hidden width, optimizer, epochs, and parameter count.",
        ...
        "training_records": slim_training_records,
        "run_table": run_table,
...
def write_stage30_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage30_table(result: dict[str, Any]) -> None:
    columns = (
        "method",
        "run_count",
        "parameter_count",
