...
STAGE17_SCHEMA_VERSION = "qrope_stage17_small_decoder_value_model_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage17_small_decoder_value_model"
EMBED_DIM = 12
...
def _init_params(method_name: str, feature_dim: int, seed: int, embed_dim: int = EMBED_DIM) -> dict[str, np.ndarray]:
    digest = hashlib.sha256(f"stage17:{method_name}:{seed}".encode("utf-8")).hexdigest()
...
def _loss_and_gradient(
    ...
    for row in rows:
        ...
        # Use a uniform multi-target cross-entropy surrogate for stable manual gradients.
        ...
        for candidate_index, token_id in enumerate(value_ids):
...
def train_value_model(
    ...
    *,
    ...
    params = _init_params(method_name, feature_dim, seed)
...
def evaluate_value_model(rows: list[Stage14Example], method_name: str, params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
...
def run_stage17_benchmark(
    *,
    ...
    for method_name in METHOD_NAMES:
        training = train_value_model(splits["train"], method_name, epochs=epochs)
        params = _params_from_record(training)
        training_records.append(training)
        row = evaluate_value_model(splits["test"], method_name, params)
        row["method"] = method_name
        table.append(row)
        for task_name in TASK_NAMES:
            task_rows = [example for example in splits["test"] if example.task == task_name]
            task_result = evaluate_value_model(task_rows, method_name, params)
    ...
    return {
        "schema_version": STAGE17_SCHEMA_VERSION,
        "stage": "stage17_small_decoder_value_model",
        "dataset": "deterministic_non_phase_cued_small_decoder_value_model_v1",
        ...
        "model": {
            "type": "single_attention_readout_with_learned_value_embeddings_and_output_projection",
            "embed_dim": EMBED_DIM,
        ...
        "claim_boundary": {
            "supported": [
                "A deterministic train-short/test-long learned value-token readout over non-phase-cued key-value rows.",
                "Evidence about whether PhaseWrap-derived features survive a learned embedding/output readout.",
            ],
            "excluded": [
...
def write_stage17_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    ...
    public_result["training_records"] = [
...
def print_stage17_table(result: dict[str, Any]) -> None:
    columns = (
        "method",
        "row_count",
        "top1_accuracy",
