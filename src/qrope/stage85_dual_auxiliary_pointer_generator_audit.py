...
STAGE85_SCHEMA_VERSION = "qrope_stage85_dual_auxiliary_pointer_generator_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage85_dual_auxiliary_pointer_generator_audit"
DEFAULT_EPOCHS = 10
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE85_SCHEMA_VERSION,
            "stage": "stage85_dual_auxiliary_pointer_generator_audit",
...
def _batch_loss(
    ...
    return anp.mean(
        anp.array(
            [
                _row_loss(vector, row, method_name, support_classes, support_aux_weight, target_attention_aux_weight)
                for row in rows
            ]
        )
    )
...
def train_dual_auxiliary_pointer_generator_decoder(
    rows: list[Stage10Example],
    method_name: str,
    *,
    seed: int,
    ...
    gradient = grad(lambda current: _batch_loss(current, rows, method_name, support_classes, support_aux_weight, target_attention_aux_weight))
    ...
    for epoch in range(1, epochs + 1):
        loss_value = float(_batch_loss(vector, rows, method_name, support_classes, support_aux_weight, target_attention_aux_weight))
...
def evaluate_dual_auxiliary_pointer_generator(rows: list[Stage10Example], method_name: str, vector: Any, support_classes: tuple[int, ...]) -> dict[str, float]:
    losses: list[float] = []
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "DUAL_AUXILIARY_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED"
        boundary = "Dual support/target-attention supervision does not establish train capacity."
    elif all(value >= GENERALIZATION_TOP1_THRESHOLD for value in retrieval_best_top1.values()):
        decision = "DUAL_AUXILIARY_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED"
        boundary = "Dual support/target-attention supervision generalizes all retrieval lanes; review method ordering before any claim update."
    elif generalized_retrieval:
        decision = "DUAL_AUXILIARY_POINTER_GENERATOR_PARTIAL_RETRIEVAL"
        boundary = "Dual support/target-attention supervision generalizes at least one retrieval lane but not the full retrieval gate."
    elif attention_repaired:
        decision = "DUAL_AUXILIARY_POINTER_GENERATOR_ATTENTION_REPAIRED_TOKEN_FAILED"
        boundary = "Dual supervision repairs target attention for at least one retrieval lane, but token retrieval remains below threshold."
    else:
        decision = "DUAL_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "Dual support/target-attention supervision preserves capacity but does not repair held-out retrieval generalization."
    return {
        **base,
        "decision": decision,
...
def run_stage85_audit(
    *,
            ...
            try:
                ...
                for task_name in TASK_NAMES:
                    ...
                    for split_name, split_rows in (("train", task_train_rows), ("validation", validation_rows), ("test", test_rows)):
                        metrics = evaluate_dual_auxiliary_pointer_generator(split_rows, stage10_method, trained["weights"], support_classes)
    ...
    result = {
        ...
        "stage": "stage85_dual_auxiliary_pointer_generator_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_support_complete_rows_multitask_dual_auxiliary_pointer_generator_v1",
        ...
        "model": {
            "type": "two_block_pointer_generator_dual_auxiliary_decoder",
            "optimizer": "full_batch_adam_dual_auxiliary",
...
def write_stage85_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage85_summary(result: dict[str, Any]) -> None:
