...
STAGE58_SCHEMA_VERSION = "qrope_stage58_pooled_train_query_support_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage58_pooled_train_query_support_audit"
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            "A pooled-train query-token support-map diagnostic over the Stage 52-57 row family.",
            "Evidence about whether the known support prior in Stage 57 can be recovered from training rows.",
...
def build_blocked_result(*, reason: str = "unexpected_preflight_block") -> dict[str, Any]:
    return {
        "schema_version": STAGE58_SCHEMA_VERSION,
        "stage": "stage58_pooled_train_query_support_audit",
        "status": "blocked",
...
def evaluate_pooled_train_query_support(
...
def _decision(aggregate_table: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if len(retrieval_solved) == len(RETRIEVAL_TASKS) and "phase_cued_retrieval" in no_position_solved:
        decision = "POOLED_TRAIN_QUERY_SUPPORT_SOLVES_PHASE_CUED_NOT_PROMOTION"
        boundary = "A pooled train lookup recovers the support-aware query cue and solves phase-cued retrieval for no-position too; this is not positional-method promotion."
    elif retrieval_solved:
        decision = "POOLED_TRAIN_QUERY_SUPPORT_PARTIAL_RETRIEVAL"
        boundary = "A pooled train lookup solves at least one retrieval lane but not the full retrieval set."
    else:
        decision = "POOLED_TRAIN_QUERY_SUPPORT_RETRIEVAL_FAILED"
        boundary = "A pooled train lookup does not solve retrieval."
...
def run_stage58_audit(
    *,
    ...
    pooled_train_rows = [row for splits in splits_by_task.values() for row in splits["train"]]
    learned_support = learn_query_support_map(pooled_train_rows)
    ...
    return {
        ...
        "no_hardware_submission": True,
        "provider_credentials_required": False,
        "source_stage": "stage57_support_aware_query_cue_audit",
        "method_names": list(method_names),
        "tasks": list(TASK_NAMES),
        "seeds": list(seeds),
        "examples_per_length": examples_per_length,
        "learned_query_support_map": {str(key): value for key, value in learned_support.items()},
        "model": {
            "type": "pooled_train_query_token_support_lookup_copy_diagnostic",
...
def write_stage58_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage56_outputs(result, output_dir)


def print_stage58_summary(result: dict[str, Any]) -> None:
