...
STAGE92_SCHEMA_VERSION = "qrope_stage92_support_binding_teacher_pointer_generator_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage92_support_binding_teacher_pointer_generator_audit"
DEFAULT_EPOCHS = 10
DEFAULT_TARGET_ATTENTION_AUX_WEIGHT = 0.6
DEFAULT_TEACHER_DISTILLATION_WEIGHT = 0.8
DEFAULT_SUPPORT_BINDING_WEIGHT = 0.8
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            ...
            "Evidence about whether a training-only support-weighted copy binding objective transfers into free evaluation without structural copy routing.",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    ...
    result.update(
        {
            "schema_version": STAGE92_SCHEMA_VERSION,
            "stage": "stage92_support_binding_teacher_pointer_generator_audit",
            ...
            "support_binding_weight": DEFAULT_SUPPORT_BINDING_WEIGHT,
...
def _support_binding_distribution(row: Stage10Example, support_classes: tuple[int, ...], support_probabilities: Any):
    import autograd.numpy as anp

    indicator = anp.array(_copy_indicator(row))
    probabilities = anp.zeros(indicator.shape[1])
    for class_index, reference_delta in enumerate(support_classes):
        ...
        selected_position = int(row.query_pos - selected_delta)
        if 0 <= selected_position < row.query_pos:
            probabilities = probabilities + support_probabilities[class_index] * indicator[selected_position]
    total = anp.sum(probabilities)
    return probabilities / (total + 1e-12)
...
def _row_loss(
    ...
    if row.task == "phase_cued_retrieval":
        ...
        binding_probabilities = _support_binding_distribution(row, support_classes, support_probabilities)
        losses.append(support_binding_weight * -anp.log(binding_probabilities[row.label_token] + 1e-12))
...
def train_three_block_teacher_distilled_pointer_generator_decoder(
    ...
    *,
    ...
    support_binding_weight: float = DEFAULT_SUPPORT_BINDING_WEIGHT,
) -> dict[str, Any]:
    ...
    return {
        ...
        "support_binding_weight": support_binding_weight,
...
def _decision(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if not base["capacity_established"]:
        decision = "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_CAPACITY_NOT_ESTABLISHED"
        boundary = "Support-binding teacher supervision does not establish train capacity."
    elif all(value >= GENERALIZATION_TOP1_THRESHOLD for value in retrieval_best_top1.values()):
        decision = "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_RETRIEVAL_REVIEW_REQUIRED"
        boundary = "Support-binding teacher supervision generalizes all retrieval lanes without evaluation-time structural routing; review method ordering before any claim update."
    elif generalized_retrieval:
        decision = "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_PARTIAL_RETRIEVAL"
        boundary = "Support-binding teacher supervision generalizes at least one retrieval lane but not the full retrieval gate."
    elif attention_repaired:
        decision = "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_ATTENTION_REPAIRED_TOKEN_FAILED"
        boundary = "Support-binding teacher supervision repairs target attention for at least one retrieval lane, but free token retrieval remains below threshold."
    else:
        decision = "SUPPORT_BINDING_TEACHER_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION"
        boundary = "Support-binding teacher supervision preserves capacity but does not repair free held-out retrieval generalization."
    return {
        **base,
        "decision": decision,
...
def run_stage92_audit(
    *,
    ...
    result = {
        ...
        "stage": "stage92_support_binding_teacher_pointer_generator_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_support_complete_rows_multitask_support_binding_teacher_pointer_generator_v1",
        ...
        "model": {
            "type": "three_block_pointer_generator_support_binding_teacher_decoder",
            "optimizer": "full_batch_adam_three_block_structural_teacher_distillation",
            ...
            "support_binding_policy": "support-weighted copied-token binding loss is applied to phase-cued train rows only; validation and test use the free learned pointer-generator distribution",
...
def write_stage92_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage52_outputs(result, output_dir)


def print_stage92_summary(result: dict[str, Any]) -> None:
