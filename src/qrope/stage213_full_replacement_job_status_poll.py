...
STAGE213_SCHEMA_VERSION = "qrope_stage213_full_replacement_job_status_poll_v1"
...
DEFAULT_OUTPUT_DIR = DEFAULT_ARTIFACT_ROOT / "stage213_full_replacement_job_status_poll_250usd"
...
def run_stage213_full_replacement_job_status_poll(
    *,
    ...
    missing_sources = [] if isinstance(stage212, dict) else [str(stage212_results_path.as_posix())]
    blockers: list[str] = []
    if missing_sources:
    ...
    submissions = [record for record in (stage212.get("submission_records", []) if isinstance(stage212, dict) else []) if record.get("runtime_job_id")]
    if len(submissions) != 21:
        blockers.append("stage212_runtime_job_count_mismatch")
    ...
    if blockers:
        decision = "FULL_REPLACEMENT_JOB_STATUS_POLL_BLOCKED_OR_INCOMPLETE"
    elif completed_count == len(poll_records) and poll_records:
        decision = "FULL_REPLACEMENT_RUNTIME_JOBS_COMPLETE_READY_FOR_RESULT_COLLECTION"
    else:
        decision = "FULL_REPLACEMENT_RUNTIME_JOBS_SUBMITTED_RESULTS_PENDING"
    return {
        "schema_version": STAGE213_SCHEMA_VERSION,
        "stage": "stage213_full_replacement_job_status_poll",
        "status": "completed" if not missing_sources else "incomplete",
        "objective": OBJECTIVE,
        "decision": decision,
        "source_artifacts": [str(stage212_results_path.as_posix())],
        ...
        "claim_boundary": {
            "supported": ["status polling for submitted full 4096-shot IBM Runtime jobs"],
            "excluded": ["new hardware job submission", "provider-side result counts", "calibration pass/fail", "robustness or auditability interpretation"],
        },
        "next_gate": "When all Runtime jobs are complete, collect provider counts and populate full replacement evidence records.",
    }


def write_stage213_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_keys = (
        "schema_version", "stage", "status", "objective", "decision", "source_artifacts",
        "missing_source_artifacts", "blockers", "expected_runtime_job_count",
        "observed_runtime_job_count", "polled_runtime_job_count", "completed_runtime_job_count",
        "failed_runtime_job_count", "no_hardware_submission", "provider_credentials_required",
        "secret_values_recorded", "runnable_commands_recorded", "claim_boundary", "next_gate",
    )
...
def print_stage213_summary(result: dict[str, Any]) -> None:
