...
STAGE182_SCHEMA_VERSION = "qrope_stage182_balanced_phase_window_candidate_screen_v1"
...
DEFAULT_OUTPUT_DIR = DEFAULT_ARTIFACT_ROOT / "stage182_balanced_phase_window_candidate_screen"
DESIGN_FAMILY_ID = "pw_balanced_phase_window_v1"
...
def _balanced_components_for_family(family: str, delta: float, max_abs_delta: float) -> tuple[float, float]:
    normalized = _normalized_delta(delta, max_abs_delta)
    phase = math.pi * normalized
    if family == "phasewrap":
        return _clamp(math.sin(phase)), _clamp(math.sin(0.5 * phase))
    if family == "rope_like":
        return _clamp(math.cos(0.25 * phase)), _clamp(math.cos(0.125 * phase))
    if family == "sinusoidal_like":
        return _clamp(math.sin(2.0 * phase)), _clamp(math.sin(phase))
    if family == "alibi_like":
        component = _clamp(1.0 - 2.0 * abs(normalized))
...
def _candidate_row(source_row: dict[str, Any], family: str, max_abs_delta: float, circuit_template: str) -> dict[str, Any]:
    ...
    component_a, component_b = _balanced_components_for_family(family, delta, max_abs_delta)
...
def _packet_for_lane_family(lane: SourceLane, family: str, circuit_template: str) -> dict[str, Any]:
    ...
    packet_core = {
        "schema_version": STAGE182_SCHEMA_VERSION,
        "packet_version": "qrope_stage182_balanced_phase_window_candidate_packet_v1",
        ...
        "matching_policy": {
            ...
            "component_transform": "balanced normalized delta window applied before RY preparation",
        ...
        "claim_boundary": (
            "Balanced phase-window candidate packet for simulated IBM-informed screening only; "
...
def run_stage182_balanced_phase_window_candidate_screen(
    *,
    ...
    if isinstance(stage181, dict):
        ...
        if DESIGN_FAMILY_ID not in family_ids:
            blockers.append("stage181_balanced_phase_window_family_missing")
    ...
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="qrope_stage182_packets_") as temp_dir:
    ...
    if blockers:
        decision = "BALANCED_PHASE_WINDOW_CANDIDATE_SCREEN_INCOMPLETE"
    elif reopen["supports_hardware_reopen"]:
        decision = "BALANCED_PHASE_WINDOW_CANDIDATE_SUPPORTS_HARDWARE_REOPEN"
    else:
        decision = "BALANCED_PHASE_WINDOW_CANDIDATE_DOES_NOT_SUPPORT_HARDWARE_REOPEN"

    return {
        "schema_version": STAGE182_SCHEMA_VERSION,
        "stage": "stage182_balanced_phase_window_candidate_screen",
        ...
        "claim_boundary": {
            "supported": [
                "simulated-only first redesign screen for the Stage181 balanced phase-window family",
...
def write_stage182_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage182_summary(result: dict[str, Any]) -> None:
