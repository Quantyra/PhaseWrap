...
STAGE83_SCHEMA_VERSION = "qrope_stage83_nonlinear_support_routing_bridge_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage83_nonlinear_support_routing_bridge_audit"
DEFAULT_AUDIT_SEEDS = DEFAULT_SEEDS
DEFAULT_ROUTING_EPOCHS = 160
DEFAULT_ROUTING_LEARNING_RATE = 0.03
DEFAULT_ROUTING_HIDDEN_WIDTH = 8
ROUTING_FEATURE_NAMES = (
    "position_bias",
    "learned_support_congruence",
    "normalized_distance",
    "position_bias_x_support",
    "support_x_distance",
    "position_bias_x_distance",
    "support_squared",
    "distance_squared",
)
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            "A no-credential same-seed support-complete audit that trains a nonlinear per-position routing bridge over learned support probabilities.",
            "Evidence about whether Stage82's compact scalar routing failure is explained by insufficient learned routing capacity.",
        ...
        "excluded": [
            ...
            "a claim that this nonlinear routing bridge is a matched decoder-only transformer",
            "a claim that nonlinear support routing is positional-method promotion evidence when no-position solves too",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    return {
        "schema_version": STAGE83_SCHEMA_VERSION,
        "stage": "stage83_nonlinear_support_routing_bridge_audit",
        "status": "blocked",
        "blocked_reason": reason,
        "install_command": "python -m pip install -e \".[transformer]\"",
        "no_hardware_submission": True,
        "provider_credentials_required": False,
        "source_stage": "stage82_learned_support_routing_head_audit",
...
def _routing_features(row: Stage10Example, method_name: str, model: dict[str, Any]):
    import autograd.numpy as anp

    position_bias = anp.array(positional_bias(row, method_name))
    support = _support_scores(row, model)
    distances = anp.array([row.query_pos - index for index in range(row.query_pos)], dtype=float)
    normalized_distance = distances / max(1.0, float(row.query_pos))
    return anp.stack(
        [
            position_bias,
            support,
            normalized_distance,
            position_bias * support,
            support * normalized_distance,
            position_bias * normalized_distance,
            support * support,
...
def _unpack_route_bridge(route_vector: Any, *, input_width: int, hidden_width: int):
    import autograd.numpy as anp

    cursor = 0
    w1_size = input_width * hidden_width
    w1 = anp.reshape(route_vector[cursor : cursor + w1_size], (input_width, hidden_width))
    cursor += w1_size
    b1 = route_vector[cursor : cursor + hidden_width]
    cursor += hidden_width
    w2 = route_vector[cursor : cursor + hidden_width]
    cursor += hidden_width
    b2 = route_vector[cursor]
    return w1, b1, w2, b2


def _routing_attention(route_vector: Any, row: Stage10Example, method_name: str, model: dict[str, Any], *, hidden_width: int):
    import autograd.numpy as anp

    features = _routing_features(row, method_name, model)
    w1, b1, w2, b2 = _unpack_route_bridge(route_vector, input_width=len(ROUTING_FEATURE_NAMES), hidden_width=hidden_width)
    hidden = anp.tanh(anp.dot(features, w1) + b1)
    logits = anp.dot(hidden, w2) + b2
    return _softmax(logits)


def _routing_row_probabilities(route_vector: Any, row: Stage10Example, method_name: str, model: dict[str, Any], *, hidden_width: int):
    import autograd.numpy as anp

    attention = _routing_attention(route_vector, row, method_name, model, hidden_width=hidden_width)
    return anp.dot(attention, _copy_indicator(row))


def _routing_loss(route_vector: Any, rows: list[Stage10Example], method_name: str, model: dict[str, Any], *, hidden_width: int):
    import autograd.numpy as anp

    losses = []
    for row in rows:
        probabilities = _routing_row_probabilities(route_vector, row, method_name, model, hidden_width=hidden_width)
        losses.append(-anp.log(probabilities[row.label_token] + 1e-12))
    return anp.mean(anp.array(losses))


def train_nonlinear_support_routing_bridge(
    ...
    *,
    ...
    hidden_width: int = DEFAULT_ROUTING_HIDDEN_WIDTH,
    random_seed: int = 0,
) -> dict[str, Any]:
    ...
    input_width = len(ROUTING_FEATURE_NAMES)
    vector = np.concatenate(
        [
            rng.normal(0.0, 0.05, size=input_width * hidden_width),
            np.zeros(hidden_width, dtype=float),
            rng.normal(0.0, 0.05, size=hidden_width),
            np.zeros(1, dtype=float),
        ]
    )
    gradient = grad(lambda current: _routing_loss(current, route_rows, method_name, model, hidden_width=hidden_width))
    ...
    for epoch in range(1, epochs + 1):
        loss_value = float(_routing_loss(vector, route_rows, method_name, model, hidden_width=hidden_width))
    ...
    w1, _, w2, _ = _unpack_route_bridge(vector, input_width=input_width, hidden_width=hidden_width)
    first_layer = np.asarray(w1, dtype=float)
    output_layer = np.asarray(w2, dtype=float)
    feature_norms = np.linalg.norm(first_layer * output_layer.reshape(1, -1), axis=1)
    return {
        "weights": vector,
        "optimizer": "full_batch_adam_nonlinear_route_bridge",
        "epochs": epochs,
        ...
        "diagnostic_position_weight_norm": round(float(feature_norms[0]), 6),
        "diagnostic_support_weight_norm": round(float(feature_norms[1]), 6),
        "diagnostic_distance_weight_norm": round(float(feature_norms[2]), 6),
    }
...
def _routed_row_probabilities(row: Stage10Example, method_name: str, model: dict[str, Any], routing_head: dict[str, Any]) -> np.ndarray:
    if row.task == "phase_cued_retrieval":
        return np.asarray(_routing_row_probabilities(routing_head["weights"], row, method_name, model, hidden_width=routing_head["hidden_width"]), dtype=float)
...
def evaluate_nonlinear_support_routing_bridge(rows: list[Stage10Example], method_name: str, model: dict[str, Any], routing_head: dict[str, Any]) -> dict[str, float]:
...
def _decision(aggregate_table: list[dict[str, Any]], *, support_summary: dict[str, dict[str, float]]) -> dict[str, Any]:
    ...
    if "phase_cued_retrieval" in retrieval_solved and "phase_cued_retrieval" in no_position_solved:
        decision = "NONLINEAR_SUPPORT_ROUTING_BRIDGE_SOLVES_PHASE_CUED_NOT_PROMOTION"
        boundary = "A nonlinear learned support-to-token routing bridge repairs phase-cued retrieval for no-position too; this is a learned coupling diagnostic, not positional-method promotion."
    elif "phase_cued_retrieval" in retrieval_solved:
        decision = "NONLINEAR_SUPPORT_ROUTING_BRIDGE_PHASE_CUED_REVIEW_REQUIRED"
        boundary = "A nonlinear learned support-to-token routing bridge repairs phase-cued retrieval without no-position crossing threshold; review method ordering before any claim update."
    elif mean_support_accuracy >= 1.0:
        decision = "NONLINEAR_SUPPORT_ROUTING_BRIDGE_SUPPORT_RECOVERED_RETRIEVAL_FAILED"
        boundary = "Support labels are recovered, but nonlinear learned support-to-token routing still does not repair phase-cued retrieval."
    else:
        decision = "NONLINEAR_SUPPORT_ROUTING_BRIDGE_SUPPORT_NOT_RECOVERED"
        boundary = "Support labels are not fully recovered under the nonlinear support-routing bridge audit."
...
def run_stage83_audit(
    *,
    ...
    routing_hidden_width: int = DEFAULT_ROUTING_HIDDEN_WIDTH,
) -> dict[str, Any]:
    ...
    for seed in seeds:
        ...
        for method_name in method_names:
            try:
                ...
                routing_head = train_nonlinear_support_routing_bridge(
                    train_rows_all,
                    stage10_method,
                    model,
                    epochs=routing_epochs,
                    learning_rate=routing_learning_rate,
                    hidden_width=routing_hidden_width,
                    random_seed=seed + sum(ord(character) for character in method_name),
                )
                ...
                for task_name, splits in splits_by_task.items():
                    ...
                    run: dict[str, Any] = {
                        ...
                        "selected_position_scale": routing_head["diagnostic_position_weight_norm"],
                        "selected_cue_scale": routing_head["diagnostic_support_weight_norm"],
                        "selected_distance_scale": routing_head["diagnostic_distance_weight_norm"],
                        "routing_position_weight_norm": routing_head["diagnostic_position_weight_norm"],
                        "routing_support_weight_norm": routing_head["diagnostic_support_weight_norm"],
                        "routing_distance_weight_norm": routing_head["diagnostic_distance_weight_norm"],
                    }
                    for split_name, split_rows in (("train", train_rows), ("validation", validation_rows), ("test", test_rows)):
                        metrics = evaluate_nonlinear_support_routing_bridge(split_rows, stage10_method, model, routing_head)
    ...
    return {
        ...
        "stage": "stage83_nonlinear_support_routing_bridge_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_original_rows_same_seed_support_complete_nonlinear_routing_bridge_v1",
        "no_hardware_submission": True,
        "provider_credentials_required": False,
        "source_stage": "stage82_learned_support_routing_head_audit",
        "method_names": list(method_names),
        ...
        "routing_hidden_width": routing_hidden_width,
        "routing_feature_names": list(ROUTING_FEATURE_NAMES),
        ...
        "model": {
            "type": "same_seed_support_complete_nonlinear_support_routing_bridge",
            "value_output_mode": "deterministic copied prefix-token mass with a nonlinear learned per-position bridge over support-congruence, positional-bias, distance, and interaction features for phase-cued rows",
...
def write_stage83_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage56_outputs(result, output_dir)


def print_stage83_summary(result: dict[str, Any]) -> None:
