...
def generate_symbolic_insufficiency_echo_resolution_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_symbolic_insufficiency_echo_resolution_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    candidate_rows = list(all_rows)
    rng.shuffle(candidate_rows)
    candidate_rows = sorted(candidate_rows[:22], key=lambda row: row.text)

    candidates_by_state: dict[tuple[int, int, int, int], list[dict[str, Any]]] = defaultdict(list)
    for index_s, row_s in enumerate(candidate_rows):
        for index_e1, row_e1 in enumerate(candidate_rows):
            if index_e1 == index_s:
                continue
            for index_e2, row_e2 in enumerate(candidate_rows):
                if index_e2 in {index_s, index_e1}:
                    continue
                for index_r, row_r in enumerate(candidate_rows):
                    if index_r in {index_s, index_e1, index_e2}:
                        continue
                    source_sign = int(
                        sector_sign_family(row_s.sector_a) == sector_sign_family(row_s.sector_b)
                    )
                    echo_one_gate = int(
                        token_orientation_name(row_e1.sample_a.left_token, row_e1.sample_a.right_token)
                        == token_orientation_name(row_s.sample_a.left_token, row_s.sample_a.right_token)
                    )
                    echo_two_gate = int(
                        content_family_name(row_e2.sample_a.left_token, row_e2.sample_a.right_token)
                        == content_family_name(row_e1.sample_a.left_token, row_e1.sample_a.right_token)
                    )
                    resolve_bind = int(
                        token_orientation_name(row_r.sample_b.left_token, row_r.sample_b.right_token)
                        == token_orientation_name(row_s.sample_b.left_token, row_s.sample_b.right_token)
                    )
                    coarse_key = (source_sign, echo_one_gate, echo_two_gate, resolve_bind)

                    latent_s = symbolic_insufficiency_latent_ids(row_s.sample_a, row_s.sample_b)
                    latent_e1 = symbolic_insufficiency_latent_ids(row_e1.sample_a, row_e1.sample_b)
                    latent_e2 = symbolic_insufficiency_latent_ids(row_e2.sample_a, row_e2.sample_b)
                    latent_r = symbolic_insufficiency_latent_ids(row_r.sample_a, row_r.sample_b)
                    phase_s = _symbolic_insufficiency_latent_phase(latent_s)
                    phase_e1 = _symbolic_insufficiency_latent_phase(latent_e1)
                    phase_e2 = _symbolic_insufficiency_latent_phase(latent_e2)
                    phase_r = _symbolic_insufficiency_latent_phase(latent_r)
                    echo_orientation_mean = 0.5 * (
                        orientation_delta_score(row_e1.sample_a, row_e1.sample_b)
                        + orientation_delta_score(row_e2.sample_a, row_e2.sample_b)
                    )
                    echo_sector_mean = 0.5 * (
                        normalized_sector_magnitude_delta(row_e1.sample_a, row_e1.sample_b)
                        + normalized_sector_magnitude_delta(row_e2.sample_a, row_e2.sample_b)
                    )
                    echo_content_mean = 0.5 * (
                        ordered_content_delta_score(row_e1.sample_a, row_e1.sample_b)
                        + ordered_content_delta_score(row_e2.sample_a, row_e2.sample_b)
                    )
                    raw_target = (
                        0.18 * float(row_s.label)
                        + 0.11 * float(row_e1.label)
                        - 0.09 * float(row_e2.label)
                        + 0.21 * float(row_r.label)
                        + 0.16 * math.sin((phase_s + phase_e2) - (phase_e1 + phase_r))
                        + 0.13 * math.cos((phase_e1 - phase_s) + (phase_r - phase_e2))
                        + 0.11 * math.sin(echo_orientation_mean + phase_r - phase_s)
                        + 0.08
                        * math.cos(
                            orientation_delta_score(row_s.sample_a, row_s.sample_b)
                            + orientation_delta_score(row_e1.sample_a, row_e1.sample_b)
                        ...
                        + 0.07 * echo_sector_mean
                        + 0.06 * echo_content_mean
                    )
                    candidates_by_state[coarse_key].append(
                        {
                            "text": render_symbolic_insufficiency_echo_resolution_text(
                                row_s.sample_a,
                                row_s.sample_b,
                                row_e1.sample_a,
                                row_e1.sample_b,
                            ...
                            "raw_target": round(raw_target, 6),
                            "latent_key": (*latent_s, *latent_e1, *latent_e2, *latent_r),
                        }
                    )
    ...
    diagnostics = {
        ...
        "coarse_echo_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0) <= 1e-6,
        "within_echo_state_variation_pass": all(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "latent_echo_diversity_pass": all(value > 1 for value in latent_group_counts.values()) and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "echo_length_balance_pass": True,
        "resolution_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "echo_bucket_counts": bucket_counts,
        "coarse_echo_state_null_max_abs_mean": round(max((abs(value) for value in state_means.values()), default=0.0), 6),
        "within_echo_state_target_ranges": target_ranges,
        "latent_echo_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)


def generate_symbolic_insufficiency_selector_arbitration_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_symbolic_insufficiency_selector_arbitration_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
        ...
        for index_a, row_a in enumerate(candidate_rows):
            ...
            for index_b, row_b in enumerate(candidate_rows):
                ...
                for index_t, row_t in enumerate(candidate_rows):
                    if index_t in {index_s, index_a, index_b}:
                        continue
                    source_sign = int(
                        sector_sign_family(row_s.sector_a) == sector_sign_family(row_s.sector_b)
                    )
                    candidate_a_gate = int(
                        token_orientation_name(row_a.sample_a.left_token, row_a.sample_a.right_token)
                        == token_orientation_name(row_s.sample_a.left_token, row_s.sample_a.right_token)
                    )
                    candidate_b_gate = int(
                        token_orientation_name(row_b.sample_b.left_token, row_b.sample_b.right_token)
                        == token_orientation_name(row_s.sample_b.left_token, row_s.sample_b.right_token)
                    )
                    selector_bind = int(
                        content_family_name(row_t.sample_a.left_token, row_t.sample_a.right_token)
                        == content_family_name(row_a.sample_a.left_token, row_a.sample_a.right_token)
                    )
                    coarse_key = (source_sign, candidate_a_gate, candidate_b_gate, selector_bind)
                    ...
                    selector_pressure = normalized_sector_magnitude_delta(row_t.sample_a, row_t.sample_b)
                    candidate_gap = abs(
                        normalized_sector_magnitude_delta(row_a.sample_a, row_a.sample_b)
                        - normalized_sector_magnitude_delta(row_b.sample_a, row_b.sample_b)
                    )
                    arbitration_mix = (
                        ordered_content_delta_score(row_a.sample_a, row_a.sample_b)
                        - ordered_content_delta_score(row_b.sample_a, row_b.sample_b)
                    )
                    raw_target = (
                        0.18 * float(row_s.label)
                        + 0.14 * float(row_a.label)
                        - 0.10 * float(row_b.label)
                        + 0.20 * float(row_t.label)
                        + 0.17 * math.sin((phase_s + phase_a) - (phase_b + phase_t))
                        + 0.13 * math.cos((phase_a - phase_t) + (phase_s - phase_b))
                        + 0.10 * math.sin(arbitration_mix + selector_pressure)
                        + 0.08 * candidate_gap
                        + 0.07
                        * math.cos(
                            orientation_delta_score(row_s.sample_a, row_s.sample_b)
                    ...
                    candidates_by_state[coarse_key].append(
                        {
                            "text": render_symbolic_insufficiency_selector_arbitration_text(
                                row_s.sample_a,
                                row_s.sample_b,
                                row_a.sample_a,
                                row_a.sample_b,
                            ...
                            "raw_target": round(raw_target, 6),
                            "latent_key": (*latent_s, *latent_a, *latent_b, *latent_t),
                        }
                    )
    ...
    diagnostics = {
        ...
        "coarse_selector_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0) <= 1e-6,
        "within_selector_state_variation_pass": all(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "latent_selector_diversity_pass": all(value > 1 for value in latent_group_counts.values()) and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "selector_length_balance_pass": True,
        "selector_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "selector_bucket_counts": bucket_counts,
        "coarse_selector_state_null_max_abs_mean": round(max((abs(value) for value in state_means.values()), default=0.0), 6),
        "within_selector_state_target_ranges": target_ranges,
        "latent_selector_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)


def generate_symbolic_insufficiency_counterfactual_handoff_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_symbolic_insufficiency_counterfactual_handoff_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    candidates_by_state: dict[tuple[int, int, int, int], list[dict[str, Any]]] = defaultdict(list)
    for index_s, row_s in enumerate(candidate_rows):
        for index_h, row_h in enumerate(candidate_rows):
            if index_h == index_s:
                continue
            for index_c, row_c in enumerate(candidate_rows):
                if index_c in {index_s, index_h}:
                    continue
                for index_r, row_r in enumerate(candidate_rows):
                    if index_r in {index_s, index_h, index_c}:
                        continue
                    source_sign = int(
                        sector_sign_family(row_s.sector_a) == sector_sign_family(row_s.sector_b)
                    )
                    handoff_gate = int(
                        token_orientation_name(row_h.sample_a.left_token, row_h.sample_a.right_token)
                        == token_orientation_name(row_s.sample_a.left_token, row_s.sample_a.right_token)
                    )
                    counterfactual_override = int(
                        content_family_name(row_c.sample_a.left_token, row_c.sample_a.right_token)
                        == content_family_name(row_h.sample_b.left_token, row_h.sample_b.right_token)
                    )
                    resolve_bind = int(
                        content_family_name(row_r.sample_a.left_token, row_r.sample_a.right_token)
                        == content_family_name(row_h.sample_a.left_token, row_h.sample_a.right_token)
                    )
                    coarse_key = (
                        source_sign,
                        handoff_gate,
                    ...
                    latent_s = symbolic_insufficiency_latent_ids(row_s.sample_a, row_s.sample_b)
                    latent_h = symbolic_insufficiency_latent_ids(row_h.sample_a, row_h.sample_b)
                    latent_c = symbolic_insufficiency_latent_ids(row_c.sample_a, row_c.sample_b)
                    latent_r = symbolic_insufficiency_latent_ids(row_r.sample_a, row_r.sample_b)
                    phase_s = _symbolic_insufficiency_latent_phase(latent_s)
                    phase_h = _symbolic_insufficiency_latent_phase(latent_h)
                    phase_c = _symbolic_insufficiency_latent_phase(latent_c)
                    phase_r = _symbolic_insufficiency_latent_phase(latent_r)
                    resolve_pressure = normalized_sector_magnitude_delta(row_r.sample_a, row_r.sample_b)
                    counterfactual_gap = abs(
                        ordered_content_delta_score(row_h.sample_a, row_h.sample_b)
                        - ordered_content_delta_score(row_c.sample_a, row_c.sample_b)
                    )
                    raw_target = (
                        0.17 * float(row_s.label)
                        + 0.18 * float(row_h.label)
                        - 0.14 * float(row_c.label)
                        + 0.21 * float(row_r.label)
                        + 0.16 * math.sin((phase_s + phase_h) - (phase_c + phase_r))
                        + 0.13 * math.cos((phase_h - phase_c) + (phase_r - phase_s))
                        + 0.09 * math.sin((phase_r - phase_h) + resolve_pressure)
                        + 0.08 * counterfactual_gap
                        + 0.07
                        * math.cos(
                            orientation_delta_score(row_s.sample_a, row_s.sample_b)
                            + orientation_delta_score(row_h.sample_a, row_h.sample_b)
                            - orientation_delta_score(row_c.sample_a, row_c.sample_b)
                            - orientation_delta_score(row_r.sample_a, row_r.sample_b)
                        )
                    )
                    candidates_by_state[coarse_key].append(
                        {
                            "text": render_symbolic_insufficiency_counterfactual_handoff_text(
                                row_s.sample_a,
                                row_s.sample_b,
                                row_h.sample_a,
                                row_h.sample_b,
                            ...
                            "raw_target": round(raw_target, 6),
                            "latent_key": (*latent_s, *latent_h, *latent_c, *latent_r),
                        }
                    )
    ...
    diagnostics = {
        ...
        "coarse_handoff_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0) <= 1e-6,
        "within_handoff_state_variation_pass": all(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "latent_handoff_diversity_pass": all(value > 1 for value in latent_group_counts.values()) and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "handoff_length_balance_pass": True,
        "handoff_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "handoff_bucket_counts": bucket_counts,
        "coarse_handoff_state_null_max_abs_mean": round(max((abs(value) for value in state_means.values()), default=0.0), 6),
        "within_handoff_state_target_ranges": target_ranges,
        "latent_handoff_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)
...
def generate_positional_anchor_distance_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_positional_anchor_distance_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    for index_a, row_a in enumerate(candidate_rows):
        anchor_pivot = mean_pos(row_a.sample_a)
        for index_n, row_n in enumerate(candidate_rows):
            if index_n == index_a:
                continue
            near_gap = mean_pos(row_n.sample_a) - anchor_pivot
            near_distance = abs(near_gap)
            for index_f, row_f in enumerate(candidate_rows):
                if index_f in {index_a, index_n}:
                    continue
                far_gap = mean_pos(row_f.sample_a) - anchor_pivot
                far_distance = abs(far_gap)
                if near_distance >= far_distance:
                    continue
                if abs(far_distance - near_distance) < 0.5:
                    continue
                for index_o, row_o in enumerate(candidate_rows):
                    if index_o in {index_a, index_n, index_f}:
                        continue
                    resolve_gap = mean_pos(row_o.sample_a) - anchor_pivot
                    resolve_distance = abs(resolve_gap)
                    anchor_sign = int(
                        sector_sign_family(row_a.sector_a) == sector_sign_family(row_a.sector_b)
                    )
                    near_is_closer = int(near_distance < far_distance)
                    far_is_farther = int(far_distance > near_distance)
                    resolve_matches_distance = int(resolve_distance >= near_distance and resolve_distance <= far_distance)
                    coarse_key = (
                        anchor_sign,
                        near_is_closer,
                        far_is_farther,
                        resolve_matches_distance,
                    )

                    latent_a = symbolic_insufficiency_latent_ids(row_a.sample_a, row_a.sample_b)
                    latent_n = symbolic_insufficiency_latent_ids(row_n.sample_a, row_n.sample_b)
                    latent_f = symbolic_insufficiency_latent_ids(row_f.sample_a, row_f.sample_b)
                    latent_o = symbolic_insufficiency_latent_ids(row_o.sample_a, row_o.sample_b)
                    phase_a = _symbolic_insufficiency_latent_phase(latent_a)
                    phase_n = _symbolic_insufficiency_latent_phase(latent_n)
                    phase_f = _symbolic_insufficiency_latent_phase(latent_f)
                    phase_o = _symbolic_insufficiency_latent_phase(latent_o)
                    raw_target = (
                        0.16 * float(row_a.label)
                        + 0.15 * float(row_n.label)
                        + 0.15 * float(row_f.label)
                        + 0.18 * float(row_o.label)
                        + 0.14 * math.sin((phase_n - phase_a) - (phase_f - phase_o))
                        + 0.11 * math.cos((phase_f - phase_n) + (phase_o - phase_a))
                        + 0.10 * math.sin(resolve_distance - 0.5 * (near_distance + far_distance))
                        + 0.08 * (far_distance - near_distance)
                        + 0.08 * math.cos(resolve_gap - near_gap)
                        + 0.07
                        * math.cos(
                            orientation_delta_score(row_n.sample_a, row_n.sample_b)
                            - orientation_delta_score(row_f.sample_a, row_f.sample_b)
                            + orientation_delta_score(row_o.sample_a, row_o.sample_b)
                        )
                    )
                    candidates_by_state[coarse_key].append(
                        {
                            "text": render_positional_anchor_distance_text(
                                row_a.sample_a,
                                row_a.sample_b,
                                row_n.sample_a,
                                row_n.sample_b,
                            ...
                            "raw_target": round(raw_target, 6),
                            "latent_key": (*latent_a, *latent_n, *latent_f, *latent_o),
                        }
                    )
    ...
    diagnostics = {
        ...
        "coarse_anchor_distance_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0) <= 1e-6,
        "within_anchor_distance_state_variation_pass": all(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "latent_anchor_distance_diversity_pass": all(value > 1 for value in latent_group_counts.values()) and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "anchor_distance_length_balance_pass": True,
        "anchor_distance_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "anchor_distance_bucket_counts": bucket_counts,
        "coarse_anchor_distance_state_null_max_abs_mean": round(max((abs(value) for value in state_means.values()), default=0.0), 6),
        "within_anchor_distance_state_target_ranges": target_ranges,
        "latent_anchor_distance_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)


def generate_positional_anchor_span_membership_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_positional_anchor_span_membership_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
        ...
        for index_l, row_l in enumerate(candidate_rows):
            ...
            for index_r, row_r in enumerate(candidate_rows):
                ...
                for index_p, row_p in enumerate(candidate_rows):
                    if index_p in {index_a, index_l, index_r}:
                        continue
                    probe_center = mean_pos(row_p.sample_a)
                    probe_inside_span = int(left_center <= probe_center <= right_center)
                    for index_o, row_o in enumerate(candidate_rows):
                        if index_o in {index_a, index_l, index_r, index_p}:
                            continue
                        resolve_center = mean_pos(row_o.sample_a)
                        resolve_inside_span = int(left_center <= resolve_center <= right_center)
                        resolve_matches_probe_membership = int(resolve_inside_span == probe_inside_span)
                        anchor_sign = int(
                            sector_sign_family(row_a.sector_a) == sector_sign_family(row_a.sector_b)
                        )
                        coarse_key = (
                            ...
                            probe_inside_span,
                            resolve_inside_span,
                            resolve_matches_probe_membership,
                        )
                        ...
                        phase_o = _symbolic_insufficiency_latent_phase(latent_o)
                        probe_margin = membership_margin(probe_center, left_center, right_center)
                        resolve_margin = membership_margin(resolve_center, left_center, right_center)
                        raw_target = (
                            0.14 * float(row_a.label)
                            + 0.12 * float(row_l.label)
                            + 0.12 * float(row_r.label)
                            + 0.16 * float(row_p.label)
                            + 0.18 * float(row_o.label)
                            + 0.12 * math.sin((phase_p - phase_a) - (phase_o - phase_a))
                            + 0.10 * math.cos((phase_l - phase_a) + (phase_r - phase_a) - (phase_o - phase_p))
                            + 0.08 * probe_margin
                            + 0.08 * resolve_margin
                            + 0.08 * math.sin(resolve_margin - probe_margin)
                            + 0.06
                            * math.cos(
                                orientation_delta_score(row_l.sample_a, row_l.sample_b)
                                + orientation_delta_score(row_r.sample_a, row_r.sample_b)
                        ...
                        candidates_by_state[coarse_key].append(
                            {
                                "text": render_positional_anchor_span_membership_text(
                                    row_a.sample_a,
                                    row_a.sample_b,
                                    row_l.sample_a,
                                    row_l.sample_b,
                                ...
                                "raw_target": round(raw_target, 6),
                                "latent_key": (*latent_a, *latent_l, *latent_r, *latent_p, *latent_o),
                            }
                        )
    ...
    diagnostics = {
        ...
        "coarse_anchor_span_membership_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0) <= 1e-6,
        "within_anchor_span_membership_state_variation_pass": all(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "latent_anchor_span_membership_diversity_pass": all(value > 1 for value in latent_group_counts.values()) and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "anchor_span_membership_length_balance_pass": True,
        "anchor_span_membership_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "anchor_span_membership_bucket_counts": bucket_counts,
        "coarse_anchor_span_membership_state_null_max_abs_mean": round(max((abs(value) for value in state_means.values()), default=0.0), 6),
        "within_anchor_span_membership_state_target_ranges": target_ranges,
        "latent_anchor_span_membership_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)


def generate_positional_anchor_offset_signature_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_positional_anchor_offset_signature_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    def offset_bucket(value: float) -> int:
        distance = abs(value)
        if distance < 1.0:
            return 0
        if distance < 2.0:
    ...
    for index_a, row_a in enumerate(candidate_rows):
        ...
        for index_p, row_p in enumerate(candidate_rows):
            if index_p == index_a:
                continue
            probe_gap = mean_pos(row_p.sample_a) - anchor_pivot
            probe_side = int(probe_gap >= 0.0)
            probe_bucket = offset_bucket(probe_gap)
            for index_q, row_q in enumerate(candidate_rows):
                if index_q in {index_a, index_p}:
                    continue
                alt_gap = mean_pos(row_q.sample_a) - anchor_pivot
                if int(alt_gap >= 0.0) == probe_side and offset_bucket(alt_gap) == probe_bucket:
                    continue
                for index_o, row_o in enumerate(candidate_rows):
                    if index_o in {index_a, index_p, index_q}:
                        continue
                    resolve_gap = mean_pos(row_o.sample_a) - anchor_pivot
                    resolve_side = int(resolve_gap >= 0.0)
                    resolve_bucket = offset_bucket(resolve_gap)
                    resolve_matches_probe_signature = int(
                        resolve_side == probe_side and resolve_bucket == probe_bucket
                    )
                    anchor_sign = int(
                        sector_sign_family(row_a.sector_a) == sector_sign_family(row_a.sector_b)
                    )
                    ...
                    latent_a = symbolic_insufficiency_latent_ids(row_a.sample_a, row_a.sample_b)
                    latent_p = symbolic_insufficiency_latent_ids(row_p.sample_a, row_p.sample_b)
                    latent_q = symbolic_insufficiency_latent_ids(row_q.sample_a, row_q.sample_b)
                    latent_o = symbolic_insufficiency_latent_ids(row_o.sample_a, row_o.sample_b)
                    phase_a = _symbolic_insufficiency_latent_phase(latent_a)
                    phase_p = _symbolic_insufficiency_latent_phase(latent_p)
                    phase_q = _symbolic_insufficiency_latent_phase(latent_q)
                    phase_o = _symbolic_insufficiency_latent_phase(latent_o)
                    raw_target = (
                        0.16 * float(row_a.label)
                        + 0.17 * float(row_p.label)
                        + 0.12 * float(row_q.label)
                        + 0.18 * float(row_o.label)
                        + 0.14 * math.sin((phase_p - phase_a) - (phase_o - phase_a))
                        + 0.10 * math.cos((phase_p - phase_q) + (phase_o - phase_a))
                        + 0.09 * math.sin(resolve_gap - probe_gap)
                        + 0.07 * math.cos(abs(resolve_gap) - abs(probe_gap))
                        + 0.07 * float(resolve_matches_probe_signature)
                        + 0.06 * (offset_bucket(resolve_gap) - offset_bucket(alt_gap))
                        + 0.06
                        * math.cos(
                            orientation_delta_score(row_p.sample_a, row_p.sample_b)
                            - orientation_delta_score(row_q.sample_a, row_q.sample_b)
                            + orientation_delta_score(row_o.sample_a, row_o.sample_b)
                        )
                    )
                    candidates_by_state[coarse_key].append(
                        {
                            "text": render_positional_anchor_offset_signature_text(
                                row_a.sample_a,
                                row_a.sample_b,
                                row_p.sample_a,
                                row_p.sample_b,
                            ...
                            "raw_target": round(raw_target, 6),
                            "latent_key": (*latent_a, *latent_p, *latent_q, *latent_o),
                        }
                    )
    ...
    diagnostics = {
        ...
        "coarse_anchor_offset_signature_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0)
        <= 1e-6,
        "within_anchor_offset_signature_state_variation_pass": all(value > 0.0 for value in target_ranges.values())
        and bool(target_ranges),
        "latent_anchor_offset_signature_diversity_pass": all(value > 1 for value in latent_group_counts.values())
        and bool(latent_group_counts),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "anchor_offset_signature_length_balance_pass": True,
        "anchor_offset_signature_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values())
        and bool(target_ranges),
        "anchor_offset_signature_bucket_counts": bucket_counts,
        "coarse_anchor_offset_signature_state_null_max_abs_mean": round(
            max((abs(value) for value in state_means.values()), default=0.0), 6
        ),
        "within_anchor_offset_signature_state_target_ranges": target_ranges,
        "latent_anchor_offset_signature_group_counts": latent_group_counts,
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)
...
def generate_positional_key_query_offset_selection_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_positional_key_query_offset_selection_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    for query_index, query_row in enumerate(candidate_rows):
        ...
        for combo in itertools.combinations(remaining_indices, 4):
            ...
            if len(raw_candidates) != 4:
                continue
            if sum(item["agreement"] for item in raw_candidates) != 1:
                continue
            ...
            confusable_count = sum(
                ...
                if item["side"] == query_side and abs(item["bucket"] - query_bucket) <= 1
            )
            ordered_distractors = sorted(distractors, key=lambda item: (item["bucket"], item["gap"], item["row"].text))
            target_slot = (query_index + sum(combo)) % 4
            ordered_candidates = list(ordered_distractors)
            ordered_candidates.insert(target_slot, target_candidate)
            ...
            distractor_phases = [phase for index, phase in enumerate(phases) if index != target_slot]
            mean_distractor_phase = sum(distractor_phases) / len(distractor_phases)
            mean_distractor_gap = sum(
                float(item["gap_norm"]) for index, item in enumerate(ordered_candidates) if index != target_slot
            ) / 3.0
            target_gap_norm = float(ordered_candidates[target_slot]["gap_norm"])
            target_row = ordered_candidates[target_slot]["row"]
            orientation_mix = sum(
                orientation_delta_score(item["row"].sample_a, item["row"].sample_b) for item in ordered_candidates
            ) / 4.0
            raw_target = (
                0.16 * float(query_row.label)
                + 0.20 * float(target_row.label)
                + 0.05 * sum(float(item["row"].label) for index, item in enumerate(ordered_candidates) if index != target_slot)
                + 0.14 * math.sin((target_phase - query_phase) - (mean_distractor_phase - query_phase))
                + 0.11 * math.cos(target_gap_norm - mean_distractor_gap)
                + 0.09 * float(target_slot) / 3.0
                + 0.08 * desired_gap_norm * target_gap_norm
                - 0.07 * float(confusable_count)
                + 0.06 * math.cos(orientation_mix)
            )
            state_key = (query_sign, query_bucket, target_slot, int(confusable_count > 0))
            candidates_by_state[state_key].append(
                {
                    "text": render_positional_key_query_offset_selection_text(
                        query_row.sample_a,
                        query_row.sample_b,
                        ordered_candidates[0]["row"].sample_a,
                        ordered_candidates[0]["row"].sample_b,
                        ordered_candidates[1]["row"].sample_a,
                        ordered_candidates[1]["row"].sample_b,
                        ordered_candidates[2]["row"].sample_a,
                        ordered_candidates[2]["row"].sample_b,
                        ordered_candidates[3]["row"].sample_a,
                        ordered_candidates[3]["row"].sample_b,
                    ),
                    "raw_target": round(raw_target, 6),
    ...
    diagnostics = {
        ...
        "coarse_key_query_selection_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0)
        <= 1e-6,
        "within_key_query_selection_state_variation_pass": all(value > 0.0 for value in target_ranges.values())
        and bool(target_ranges),
        "candidate_set_nontrivial_pass": target_slot_values == {0, 1, 2, 3},
        "target_selection_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "bounded_candidate_count_pass": candidate_counts == {4},
        "key_query_selection_bucket_counts": bucket_counts,
        "coarse_key_query_selection_state_null_max_abs_mean": round(
            max((abs(value) for value in state_means.values()), default=0.0), 6
        ),
        "within_key_query_selection_state_target_ranges": target_ranges,
        "latent_key_query_selection_group_counts": latent_group_counts,
        "target_slot_values": sorted(target_slot_values),
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)


def generate_positional_dual_anchor_offset_consensus_response_bundle(
    seed: int,
    split_rotation: int = 0,
    slot_swap: int = 0,
    token_permutation: str = "identity",
    ...
    rng = random.Random(f"synthetic_positional_dual_anchor_offset_consensus_response:{seed}")
    base_bundle = generate_dual_sector_bundle(
        seed=seed,
        dataset_name="synthetic_symbolic_insufficiency_transition_response",
        split_rotation=split_rotation,
    ...
    def anchor_rule(row: DualSyntheticSample) -> tuple[int, int, float]:
        desired_gap = round(mean_pos(row.sample_b) - mean_pos(row.sample_a), 6)
        return int(desired_gap >= 0.0), gap_bucket(desired_gap), round(desired_gap / 4.0, 6)

    required = TRAIN_COUNT_PER_BUCKET + VALIDATION_COUNT_PER_BUCKET + TEST_COUNT_PER_BUCKET
    candidates_by_state: dict[tuple[int, int, int, int, int, int], list[dict[str, Any]]] = defaultdict(list)
    for anchor0_index, anchor0_row in enumerate(candidate_rows):
        anchor0_side, anchor0_bucket, anchor0_gap_norm = anchor_rule(anchor0_row)
        anchor0_pivot = mean_pos(anchor0_row.sample_a)
        for anchor1_index, anchor1_row in enumerate(candidate_rows):
            if anchor1_index == anchor0_index:
                continue
            anchor1_side, anchor1_bucket, anchor1_gap_norm = anchor_rule(anchor1_row)
            anchor1_pivot = mean_pos(anchor1_row.sample_a)
            if anchor0_side == anchor1_side and anchor0_bucket == anchor1_bucket:
                continue
            remaining_indices = [
                index for index in range(len(candidate_rows)) if index not in {anchor0_index, anchor1_index}
            ]
            for combo in itertools.combinations(remaining_indices, 4):
                raw_candidates: list[dict[str, Any]] = []
                for candidate_index in combo:
                    candidate_row = candidate_rows[candidate_index]
                    gap0 = round(mean_pos(candidate_row.sample_a) - anchor0_pivot, 6)
                    gap1 = round(mean_pos(candidate_row.sample_a) - anchor1_pivot, 6)
                    if abs(gap0) < 0.5 or abs(gap1) < 0.5:
                        raw_candidates = []
                        break
                    side0 = int(gap0 >= 0.0)
                    side1 = int(gap1 >= 0.0)
                    bucket0 = gap_bucket(gap0)
                    bucket1 = gap_bucket(gap1)
                    match0 = int(side0 == anchor0_side and bucket0 == anchor0_bucket)
                    match1 = int(side1 == anchor1_side and bucket1 == anchor1_bucket)
                    raw_candidates.append(
                        {
                            "row": candidate_row,
                            "gap0_norm": round(gap0 / 4.0, 6),
                            "gap1_norm": round(gap1 / 4.0, 6),
                            "bucket0": bucket0,
                            "bucket1": bucket1,
                            "match0": match0,
                            "match1": match1,
                            "consensus": int(match0 == 1 and match1 == 1),
                        }
                    )
                if len(raw_candidates) != 4:
                    continue
                if sum(item["consensus"] for item in raw_candidates) != 1:
                    continue
                partial_count = sum(1 for item in raw_candidates if item["match0"] + item["match1"] == 1)
                if partial_count == 0:
                    continue
                target_candidate = next(item for item in raw_candidates if item["consensus"] == 1)
                distractors = [item for item in raw_candidates if item["consensus"] == 0]
                ordered_distractors = sorted(
                    distractors,
                    key=lambda item: (
                        -(item["match0"] + item["match1"]),
                        abs(item["bucket0"] - anchor0_bucket) + abs(item["bucket1"] - anchor1_bucket),
                        item["gap0_norm"],
                        item["gap1_norm"],
                        item["row"].text,
                    ),
                )
                target_slot = (anchor0_index + anchor1_index + sum(combo)) % 4
                ordered_candidates = list(ordered_distractors)
                ordered_candidates.insert(target_slot, target_candidate)

                latent_anchor0 = symbolic_insufficiency_latent_ids(anchor0_row.sample_a, anchor0_row.sample_b)
                latent_anchor1 = symbolic_insufficiency_latent_ids(anchor1_row.sample_a, anchor1_row.sample_b)
                latent_candidates = [
                    symbolic_insufficiency_latent_ids(item["row"].sample_a, item["row"].sample_b)
                    for item in ordered_candidates
                ]
                anchor0_phase = _symbolic_insufficiency_latent_phase(latent_anchor0)
                anchor1_phase = _symbolic_insufficiency_latent_phase(latent_anchor1)
                phases = [_symbolic_insufficiency_latent_phase(latent) for latent in latent_candidates]
                target_phase = phases[target_slot]
                distractor_phases = [phase for index, phase in enumerate(phases) if index != target_slot]
                mean_distractor_phase = sum(distractor_phases) / len(distractor_phases)
                target_gap0 = float(ordered_candidates[target_slot]["gap0_norm"])
                target_gap1 = float(ordered_candidates[target_slot]["gap1_norm"])
                mean_distractor_gap0 = sum(
                    float(item["gap0_norm"]) for index, item in enumerate(ordered_candidates) if index != target_slot
                ) / 3.0
                mean_distractor_gap1 = sum(
                    float(item["gap1_norm"]) for index, item in enumerate(ordered_candidates) if index != target_slot
                ) / 3.0
                target_row = ordered_candidates[target_slot]["row"]
                orientation_mix = sum(
                    orientation_delta_score(item["row"].sample_a, item["row"].sample_b) for item in ordered_candidates
                ...
                raw_target = (
                    0.12 * float(anchor0_row.label)
                    + 0.12 * float(anchor1_row.label)
                    + 0.18 * float(target_row.label)
                    + 0.05
                    * sum(float(item["row"].label) for index, item in enumerate(ordered_candidates) if index != target_slot)
                    + 0.12 * math.sin((target_phase - anchor0_phase) - (target_phase - anchor1_phase))
                    + 0.10 * math.cos((target_gap0 - anchor0_gap_norm) - (target_gap1 - anchor1_gap_norm))
                    + 0.09 * math.sin((target_phase - mean_distractor_phase) + target_gap0 - mean_distractor_gap1)
                    + 0.07 * float(target_slot) / 3.0
                    - 0.08 * float(partial_count) / 3.0
                    + 0.07 * math.cos(orientation_mix)
                )
                state_key = (
                    anchor0_bucket,
                    anchor1_bucket,
                    int(anchor0_side == anchor1_side),
                    target_slot,
                    int(partial_count > 1),
                    int(
                        sector_sign_family(anchor0_row.sector_a) == sector_sign_family(anchor1_row.sector_a)
                    ),
                )
                candidates_by_state[state_key].append(
                    {
                        "text": render_positional_dual_anchor_offset_consensus_text(
                            anchor0_row.sample_a,
                            anchor0_row.sample_b,
                            anchor1_row.sample_a,
                            anchor1_row.sample_b,
                            ordered_candidates[0]["row"].sample_a,
                            ordered_candidates[0]["row"].sample_b,
                            ordered_candidates[1]["row"].sample_a,
                            ordered_candidates[1]["row"].sample_b,
                        ...
                        "latent_key": (
                            *latent_anchor0,
                            *latent_anchor1,
                            *(value for latent in latent_candidates for value in latent),
                        ),
                        "target_slot": target_slot,
                        "candidate_count": 4,
    ...
    for coarse_key, candidates in sorted(candidates_by_state.items()):
        ...
        target_slot_values.update(item["target_slot"] for item in selected)
        candidate_counts.update(item["candidate_count"] for item in selected)
        partial_values.update(item["partial_count"] for item in selected)
        for text, _ in centered:
            payload = parse_positional_dual_anchor_offset_consensus_text(text)
            for prefix in ("a0", "a1", "c0", "c1", "c2", "c3"):
                token_counts.update(
                    [
                        payload[prefix]["sample_a"].left_token,
                        payload[prefix]["sample_a"].right_token,
    ...
    diagnostics = {
        ...
        "coarse_dual_anchor_consensus_state_null_pass": max((abs(value) for value in state_means.values()), default=1.0)
        <= 1e-6,
        "within_dual_anchor_consensus_state_variation_pass": all(value > 0.0 for value in target_ranges.values())
        and bool(target_ranges),
        "candidate_set_nontrivial_pass": target_slot_values == {0, 1, 2, 3},
        "dual_anchor_target_nontrivial_pass": any(value > 0.0 for value in target_ranges.values()) and bool(target_ranges),
        "token_view_balance_pass": set(token_counts.keys()) == set(TOKENS),
        "bounded_candidate_count_pass": candidate_counts == {4},
        "dual_anchor_noncollapse_pass": bool(partial_values) and max(partial_values) > 0,
        "dual_anchor_consensus_bucket_counts": bucket_counts,
        "coarse_dual_anchor_consensus_state_null_max_abs_mean": round(
            max((abs(value) for value in state_means.values()), default=0.0), 6
        ),
        "within_dual_anchor_consensus_state_target_ranges": target_ranges,
        "latent_dual_anchor_consensus_group_counts": latent_group_counts,
        "target_slot_values": sorted(target_slot_values),
        "partial_match_counts": sorted(partial_values),
    }
    return SyntheticDatasetBundle(train=train, validation=validation, test=test, diagnostics=diagnostics)
...
def render_symbolic_insufficiency_echo_resolution_text(
    s_sample_a: SyntheticSample,
    s_sample_b: SyntheticSample,
    e1_sample_a: SyntheticSample,
    e1_sample_b: SyntheticSample,
    e2_sample_a: SyntheticSample,
    e2_sample_b: SyntheticSample,
    r_sample_a: SyntheticSample,
    r_sample_b: SyntheticSample,
) -> str:
    return " | ".join(
        [
            f"s:{render_dual_sample_text(s_sample_a, s_sample_b)}",
            f"e1:{render_dual_sample_text(e1_sample_a, e1_sample_b)}",
            f"e2:{render_dual_sample_text(e2_sample_a, e2_sample_b)}",
            f"r:{render_dual_sample_text(r_sample_a, r_sample_b)}",
        ]
    )


def parse_symbolic_insufficiency_echo_resolution_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
...
def render_symbolic_insufficiency_selector_arbitration_text(
    s_sample_a: SyntheticSample,
    s_sample_b: SyntheticSample,
    ...
    return " | ".join(
        [
            ...
            f"b:{render_dual_sample_text(b_sample_a, b_sample_b)}",
            f"t:{render_dual_sample_text(t_sample_a, t_sample_b)}",
        ]
    )


def parse_symbolic_insufficiency_selector_arbitration_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
...
def render_symbolic_insufficiency_counterfactual_handoff_text(
    s_sample_a: SyntheticSample,
    s_sample_b: SyntheticSample,
    h_sample_a: SyntheticSample,
    h_sample_b: SyntheticSample,
    c_sample_a: SyntheticSample,
    c_sample_b: SyntheticSample,
    r_sample_a: SyntheticSample,
    r_sample_b: SyntheticSample,
    ...
    return " | ".join(
        [
            f"s:{render_dual_sample_text(s_sample_a, s_sample_b)}",
            f"h:{render_dual_sample_text(h_sample_a, h_sample_b)}",
            f"c:{render_dual_sample_text(c_sample_a, c_sample_b)}",
            f"r:{render_dual_sample_text(r_sample_a, r_sample_b)}",
        ]
    )
...
def render_positional_anchor_distance_text(
    a_sample_a: SyntheticSample,
    a_sample_b: SyntheticSample,
    n_sample_a: SyntheticSample,
    n_sample_b: SyntheticSample,
    f_sample_a: SyntheticSample,
    f_sample_b: SyntheticSample,
    o_sample_a: SyntheticSample,
    o_sample_b: SyntheticSample,
) -> str:
    return " | ".join(
        [
            f"a:{render_dual_sample_text(a_sample_a, a_sample_b)}",
            f"n:{render_dual_sample_text(n_sample_a, n_sample_b)}",
            f"f:{render_dual_sample_text(f_sample_a, f_sample_b)}",
            f"o:{render_dual_sample_text(o_sample_a, o_sample_b)}",
        ]
    )


def parse_positional_anchor_distance_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
...
def render_positional_anchor_span_membership_text(
    a_sample_a: SyntheticSample,
    a_sample_b: SyntheticSample,
    l_sample_a: SyntheticSample,
    l_sample_b: SyntheticSample,
...
def parse_positional_anchor_span_membership_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
...
def render_positional_anchor_offset_signature_text(
    a_sample_a: SyntheticSample,
    a_sample_b: SyntheticSample,
    p_sample_a: SyntheticSample,
    p_sample_b: SyntheticSample,
    ...
    return " | ".join(
        [
            f"a:{render_dual_sample_text(a_sample_a, a_sample_b)}",
            f"p:{render_dual_sample_text(p_sample_a, p_sample_b)}",
            f"q:{render_dual_sample_text(q_sample_a, q_sample_b)}",
            f"o:{render_dual_sample_text(o_sample_a, o_sample_b)}",
        ]
    )
...
def render_positional_key_query_offset_selection_text(
    q_sample_a: SyntheticSample,
    q_sample_b: SyntheticSample,
    c0_sample_a: SyntheticSample,
    c0_sample_b: SyntheticSample,
    c1_sample_a: SyntheticSample,
    c1_sample_b: SyntheticSample,
    c2_sample_a: SyntheticSample,
    c2_sample_b: SyntheticSample,
    c3_sample_a: SyntheticSample,
    c3_sample_b: SyntheticSample,
) -> str:
    return " | ".join(
        [
            f"q:{render_dual_sample_text(q_sample_a, q_sample_b)}",
            f"c0:{render_dual_sample_text(c0_sample_a, c0_sample_b)}",
            f"c1:{render_dual_sample_text(c1_sample_a, c1_sample_b)}",
            f"c2:{render_dual_sample_text(c2_sample_a, c2_sample_b)}",
            f"c3:{render_dual_sample_text(c3_sample_a, c3_sample_b)}",
        ]
    )


def parse_positional_key_query_offset_selection_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
...
def render_positional_dual_anchor_offset_consensus_text(
    a0_sample_a: SyntheticSample,
    a0_sample_b: SyntheticSample,
    a1_sample_a: SyntheticSample,
    a1_sample_b: SyntheticSample,
    c0_sample_a: SyntheticSample,
    c0_sample_b: SyntheticSample,
    c1_sample_a: SyntheticSample,
    c1_sample_b: SyntheticSample,
    ...
    return " | ".join(
        [
            f"a0:{render_dual_sample_text(a0_sample_a, a0_sample_b)}",
            f"a1:{render_dual_sample_text(a1_sample_a, a1_sample_b)}",
            f"c0:{render_dual_sample_text(c0_sample_a, c0_sample_b)}",
            f"c1:{render_dual_sample_text(c1_sample_a, c1_sample_b)}",
            f"c2:{render_dual_sample_text(c2_sample_a, c2_sample_b)}",
            f"c3:{render_dual_sample_text(c3_sample_a, c3_sample_b)}",
...
def parse_positional_dual_anchor_offset_consensus_text(text: str) -> dict[str, Any]:
    parts = [part.strip() for part in text.split("|")]
    payloads: dict[str, dict[str, Any]] = {}
    for part in parts:
        prefix, dual_text = part.split(":", 1)
