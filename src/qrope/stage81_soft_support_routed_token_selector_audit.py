...
STAGE81_SCHEMA_VERSION = "qrope_stage81_soft_support_routed_token_selector_audit_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage81_soft_support_routed_token_selector_audit"
...
def _claim_boundary() -> dict[str, list[str]]:
    return {
        "supported": [
            "A no-credential same-seed support-complete audit that routes learned support probabilities into token selection.",
            "Evidence about whether Stage80's hard support argmax can be softened while preserving phase-cued repair.",
        ...
        "excluded": [
            ...
            "a claim that this soft support-routed selector is a matched decoder-only transformer",
...
def build_blocked_result(*, reason: str = "missing_optional_autograd_dependency") -> dict[str, Any]:
    return {
        "schema_version": STAGE81_SCHEMA_VERSION,
        "stage": "stage81_soft_support_routed_token_selector_audit",
        "status": "blocked",
        "blocked_reason": reason,
        "install_command": "python -m pip install -e \".[transformer]\"",
        "no_hardware_submission": True,
        "provider_credentials_required": False,
        "source_stage": "stage80_support_routed_token_selector_audit",
...
def _soft_support_routed_probabilities(row: Stage10Example, model: dict[str, Any]) -> np.ndarray:
    ...
    for class_index, reference_delta in enumerate(model["classes"]):
        ...
        if 0 <= selected_position < row.query_pos:
            probabilities += float(support_probabilities[class_index]) * indicator[selected_position]
...
def _routed_row_probabilities(row: Stage10Example, method_name: str, model: dict[str, Any]) -> np.ndarray:
    if row.task == "phase_cued_retrieval":
        return _soft_support_routed_probabilities(row, model)
...
def evaluate_soft_support_routed_token_selector(rows: list[Stage10Example], method_name: str, model: dict[str, Any]) -> dict[str, float]:
...
def _decision(aggregate_table: list[dict[str, Any]], *, support_summary: dict[str, dict[str, float]]) -> dict[str, Any]:
    ...
    if "phase_cued_retrieval" in retrieval_solved and "phase_cued_retrieval" in no_position_solved:
        decision = "SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION"
        boundary = "Routing learned support probabilities into token selection repairs phase-cued retrieval for no-position too; this is a coupling diagnostic, not positional-method promotion."
    elif "phase_cued_retrieval" in retrieval_solved:
        decision = "SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_PHASE_CUED_REVIEW_REQUIRED"
        boundary = "Routing learned support probabilities into token selection repairs phase-cued retrieval without no-position crossing threshold; review method ordering before any claim update."
    elif mean_support_accuracy >= 1.0:
        decision = "SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SUPPORT_RECOVERED_RETRIEVAL_FAILED"
        boundary = "Support labels are recovered, but soft support-to-token routing still does not repair phase-cued retrieval."
    else:
        decision = "SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SUPPORT_NOT_RECOVERED"
        boundary = "Support labels are not fully recovered under the soft support-routed selector audit."
...
def run_stage81_audit(
    *,
            ...
            try:
                ...
                for task_name, splits in splits_by_task.items():
                    ...
                    for split_name, split_rows in (("train", train_rows), ("validation", validation_rows), ("test", test_rows)):
                        metrics = evaluate_soft_support_routed_token_selector(split_rows, _stage10_method_name(method_name), model)
    ...
    return {
        ...
        "stage": "stage81_soft_support_routed_token_selector_audit",
        "status": "completed",
        "dataset": "synthetic_stage10_original_rows_same_seed_support_complete_soft_routed_selector_v1",
        "no_hardware_submission": True,
        "provider_credentials_required": False,
        "source_stage": "stage80_support_routed_token_selector_audit",
        "method_names": list(method_names),
        ...
        "model": {
            "type": "same_seed_support_complete_soft_support_routed_token_selector",
            "value_output_mode": "deterministic copied prefix-token mass with learned support probabilities routed to farthest congruent token selection for phase-cued rows",
            "metadata_excluded": ["hard query-support lookup", "hard support argmax", "standalone pretrained support head", "row.reference_delta exact value at evaluation", "row.target_pos", "row.target_delta"],
        },
        "claim_boundary": _claim_boundary(),
...
def write_stage81_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
    return write_stage56_outputs(result, output_dir)


def print_stage81_summary(result: dict[str, Any]) -> None:
