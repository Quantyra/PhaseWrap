...
from .stage39_sequence_decoder_retrieval import FEATURE_DIM, HIDDEN_DIM, sequence_decoder_features, sequence_tokens
from .stage40_sequence_length_curriculum import DEFAULT_CONTEXT_LENGTHS, TEST_LENGTHS, TRAIN_LENGTHS, VALIDATION_LENGTHS, split_by_curriculum_lengths


STAGE41_SCHEMA_VERSION = "qrope_stage41_pointer_copy_sequence_v1"
DEFAULT_OUTPUT_DIR = Path("logs") / "automated_stage_gates" / "stage41_pointer_copy_sequence"
DEFAULT_MODEL_SEEDS = (4103, 4111, 4127, 4133, 4139)
DEFAULT_EPOCHS = 120
...
class PreparedPointerRow:
    row: Stage14Example
    features: np.ndarray
    token_ids: np.ndarray
    target_token_indicator: np.ndarray


def prepare_pointer_rows(rows: list[Stage14Example], method_name: str) -> list[PreparedPointerRow]:
    prepared_rows: list[PreparedPointerRow] = []
    for row in rows:
        token_ids = sequence_tokens(row)
        target_values = np.array(row.target_values, dtype=int)
        prepared_rows.append(
            PreparedPointerRow(
                row=row,
                features=sequence_decoder_features(row, method_name),
                token_ids=token_ids,
                target_token_indicator=np.isin(token_ids, target_values).astype(float),
            )
        )
...
def _init_params(*, method_name: str, model_seed: int, hidden_dim: int) -> dict[str, np.ndarray]:
    seed_text = f"stage41:{method_name}:{model_seed}:{FEATURE_DIM}:{hidden_dim}"
...
def _forward(prepared: PreparedPointerRow, params: dict[str, np.ndarray]) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    hidden_pre = prepared.features @ params["w1"] + params["b1"]
    hidden = np.tanh(hidden_pre)
    logits = hidden @ params["w2"] + float(params["b2"][0])
    attention = _softmax(logits)
    values = _copy_distribution(prepared.token_ids, attention)
    return attention, values, {"hidden": hidden}


def _loss_and_gradient(rows: list[PreparedPointerRow], params: dict[str, np.ndarray], l2: float) -> tuple[float, dict[str, np.ndarray]]:
    grads = {key: np.zeros_like(value) for key, value in params.items()}
    total_loss = 0.0
    for prepared in rows:
        attention, _, cache = _forward(prepared, params)
        target_mass = max(float(attention @ prepared.target_token_indicator), 1e-12)
        total_loss += -math.log(target_mass)
        grad_attention = -prepared.target_token_indicator / target_mass
        grad_logits = attention * (grad_attention - float(attention @ grad_attention))
        hidden = cache["hidden"]
...
def train_pointer_copy_sequence(
    rows: list[PreparedPointerRow],
    method_name: str,
    *,
    model_seed: int,
...
def evaluate_pointer_copy_sequence(rows: list[PreparedPointerRow], params: dict[str, np.ndarray]) -> dict[str, Any]:
    losses: list[float] = []
    ...
    ranks: list[int] = []
    for prepared in rows:
        attention, values, _ = _forward(prepared, params)
        target_values = prepared.row.target_values
        target_mass = max(float(np.sum(values[list(target_values)])), 1e-12)
        rank = _first_relevant_rank(values, target_values)
        ...
        ranks.append(rank)
        if abs(target_mass - float(attention @ prepared.target_token_indicator)) > 1e-9:
            raise RuntimeError("copy target mass mismatch")
    ...
    return {
        ...
        "mean_target_attention_probability": round(float(np.mean(target_masses)), 6),
...
def _aggregate_runs(run_rows: list[dict[str, Any]], *, method_name: str, hidden_dim: int) -> dict[str, Any]:
    metric_names = (
        "loss",
        "top1_accuracy",
        "mrr",
    ...
    for metric_name in metric_names:
        ...
        ci = _metric_ci(values, seed_text=f"stage41:{method_name}:{metric_name}")
...
def run_stage41_benchmark(
    *,
    ...
    for method_name in method_names:
        prepared_splits = {split_name: prepare_pointer_rows(split_rows, method_name) for split_name, split_rows in raw_splits.items()}
        for model_seed in model_seeds:
            training = train_pointer_copy_sequence(
            ...
            for split_name, target_table in (("train", train_table), ("validation", validation_table), ("test", run_table)):
                row = evaluate_pointer_copy_sequence(prepared_splits[split_name], params)
            ...
            for task_name in TASK_NAMES:
                ...
                task_result = evaluate_pointer_copy_sequence(task_rows, params)
                task_result["method"] = method_name
                task_result["model_seed"] = model_seed
                task_result["task"] = task_name
                task_table.append(task_result)
    table = [_aggregate_runs([row for row in run_table if row["method"] == method_name], method_name=method_name, hidden_dim=hidden_dim) for method_name in method_names]
    train_summary = [_aggregate_runs([row for row in train_table if row["method"] == method_name], method_name=method_name, hidden_dim=hidden_dim) for method_name in method_names]
    validation_summary = [_aggregate_runs([row for row in validation_table if row["method"] == method_name], method_name=method_name, hidden_dim=hidden_dim) for method_name in method_names]
    ...
    return {
        "schema_version": STAGE41_SCHEMA_VERSION,
        "stage": "stage41_pointer_copy_sequence",
        "dataset": "stage14_full_prefix_pointer_copy_sequence_v1",
        ...
        "model": {
            "type": "single_query_sequence_decoder_pointer_copy_head",
            "feature_dim": FEATURE_DIM,
            "hidden_dim": hidden_dim,
            ...
            "value_output_mode": "copy attention mass to observed prefix token ids",
        },
        "task": {
            "description": "Pointer/copy-head repair attempt for the Stage 40 all-prefix sequence decoder.",
            "target_construction": "Targets are explicit Stage 12 retrieval-rule value tokens, not PhaseWrap-selected labels.",
            "scope": "This tests whether copy-style value output restores sequence length generalization under all-prefix attention.",
        },
        "claim_boundary": {
            "supported": [
                "A deterministic pointer/copy-head diagnostic for the all-prefix compact sequence decoder.",
                "Evidence about whether copy-style value output repairs validation/test length generalization.",
...
def write_stage41_outputs(result: dict[str, Any], output_dir: Path = DEFAULT_OUTPUT_DIR) -> dict[str, str]:
...
def print_stage41_table(result: dict[str, Any]) -> None:
