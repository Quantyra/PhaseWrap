...
| 70 | Stronger decoder-only transformer | Redesign beyond Stage 71 enough to improve original retrieval-row generalization before evaluating positional-method promotion. |
| 71 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 71 shows that a hard positional-bias copy upper bound can identify exact-offset rows for `rope_relative`, but still does not solve phase-cued retrieval. The next useful work is a stronger matched learned decoder or original-row mechanism that specifically improves phase-cued retrieval generalization before evaluating positional-method differences.
