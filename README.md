...
| 56 | Stronger decoder-only transformer | Redesign beyond Stage 57 enough to learn the support-aware cue rule from standard inputs, then rerun the same fair comparison. |
| 57 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 57 shows the phase-cued signal is visible with a known support prior, but also solves for `no_position`. The next useful work is a matched learned decoder that infers this support-aware cue rule from standard inputs before positional-method claims are expanded.
