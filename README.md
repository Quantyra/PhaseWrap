...
| 75 | Stronger decoder-only transformer | Redesign beyond Stage 76 enough to preserve visible-cue recovery inside a matched decoder-only mechanism before evaluating positional-method promotion. |
| 76 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 76 shows that integrated support/copy training does not preserve the Stage 75 phase-cued repair. The next useful work is hardening the integrated learned objective or moving to a matched decoder-only mechanism that preserves visible-cue recovery before evaluating positional-method differences.
