...
| 53 | Stronger decoder-only transformer | Redesign beyond the Stage 54 auxiliary-loss two-block path enough to generalize on retrieval tasks, then rerun the same fair comparison. |
| 54 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 54 shows that target-attention supervision of the two-block path still leaves held-out retrieval attention and top-1 unrepaired. The next useful work is a redesign beyond this autograd two-block auxiliary-loss path before positional-method claims are expanded.
