...
| 57 | Stronger decoder-only transformer | Redesign beyond Stage 58 enough to learn the support-aware cue rule inside a matched decoder, then rerun the same fair comparison. |
| 58 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 58 shows the phase-cued support map is recoverable from pooled train rows, but still solves for `no_position`. The next useful work is a matched learned decoder that internalizes this support-aware cue rule before positional-method claims are expanded.
