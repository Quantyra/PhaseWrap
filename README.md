...
| 67 | Stronger decoder-only transformer | Redesign beyond Stage 68 enough to improve original retrieval-row generalization before evaluating positional-method promotion. |
| 68 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 68 shows that even adding visible content-key auxiliary train rows does not repair the original phase-cued/exact-offset retrieval blocker. The next useful work is a stronger matched learned decoder or original-row mechanism that improves retrieval generalization before evaluating positional-method differences.
