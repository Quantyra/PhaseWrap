...
It does not need a quantum computer to compute the score. The default `SQR = m8 * m12` value is a closed-form classical feature that can be computed exactly on CPU; Stage 5 shows that the original synthetic label is exactly recoverable by mod-24 lookup and direct `m8*m12` features.

The hardware lane is therefore framed as a bounded noisy-readout witness and provider-audit probe. It tests whether frozen phase-feature packets, known-state calibration, provider bitstring decoding, raw counts, and offline metric interpretation can preserve the predeclared ordering on real devices. It is not evidence of computational speedup.

A stronger hardware-specific claim would require a preregistered same-packet comparison where classical `m8*m12`/mod-24 lookup, product-state readout, and CX readout are evaluated under the same seeds, shots, and metrics, and where the hardware-specific lane adds evidence not reducible to the classical feature baseline. That is outside the negative-results publication package.

## Current Limitations

- The fixed 8/12 score is compact and aliased: Stage 11 shows mod-24 periodicity, mirror aliases, only 10 distinct residue scores, and small Fourier support.
- Stage 5 is a motivation constraint, not a success claim: mod-24 lookup and direct `m8*m12` features exactly recover the original synthetic label.
- On non-phase-cued retrieval, fixed PhaseWrap does not beat RoPE-like or sinusoidal baselines: Stage 12 and Stage 22 are intentionally negative for the raw score.
- Learned PhaseWrap distance/multiscale adapters can recover top-1/MRR in selected bridge settings, but RoPE-like scoring often keeps stronger target probability and calibration.
- The hardware evidence is a bounded readout/witness result for frozen two-qubit packets. It is not quantum acceleration, not transformer-scale validation, and not broad cross-backend robustness.
- Stages 80 and 81 show the most important methodology warning: support-routed repairs solve phase-cued retrieval for `no_position` too, so the assistance pipeline can do the work independently of the positional method.
- The line is abandoned as a positive replacement program. The publication target is negative results plus reusable methodology.
...
- `Hardware posture`: IBM Fez product-state, IBM Fez CX, Amazon Braket/Rigetti product-state, Amazon Braket CX lanes, and the full 4096-shot IBM Fez replacement comparison have completed hardware artifacts. The replacement comparison is positive across product-state and CX templates, seeds 314 and 577, and matched positional/null controls. Additional IBM machines are not promoted to broad robustness evidence; Amazon Braket/IonQ was checked on 2026-05-19 and was not run because Forte devices were `OFFLINE` and Aria 1 was `RETIRED`; AQT IBEX Q1 is deferred due cost.
...
- `Negative-results roadmap`: [Negative results publication roadmap](docs/publication/negative-results-publication-roadmap-v1.md) prioritizes one essential retrieval-benchmark methodology paper, with score theory as appendix/optional note and evidence hygiene as an optional follow-on.
...
- The full 4096-shot IBM Fez replacement run supports a bounded two-qubit hardware claim: PhaseWrap has lower normalized noise-sensitivity delta than the best matched positional baseline and the matched null control in all four seed/template groups.
...
- The full replacement evidence is not a production transformer result, not a RoPE-replacement proof, and not a quantum-advantage claim.
...
The active publication north star is to make the negative results reproducible and useful: fixed-period score theory, method-nonspecific support-routing repairs, calibration/probability degradation in bridge settings, and reusable hardware-audit infrastructure.

## Key documents

- [Documentation landing page](docs/index.md)
- [Negative results publication roadmap](docs/publication/negative-results-publication-roadmap-v1.md)
...
- [Public review roadmap](docs/roadmap.md)
- [Scoring API reference](docs/api/scoring.md)
...
- [Publication figures index](docs/publication/figures/README.md)
...
- [Standard benchmark protocol](docs/research/q-rope-standard-benchmark-protocol-v1.md)
- [Hardware-specific claim test protocol](docs/research/q-rope-hardware-specific-claim-test-v1.md)
- [Stage 219 ranking-parity bridge](docs/research/q-rope-stage219-rope-substitution-gate-v1.md)
...
- [Stage 76 integrated support copy-head audit](docs/research/q-rope-stage76-integrated-support-copy-head-audit-v1.md)
- [Stage 77 auxiliary support copy-head audit](docs/research/q-rope-stage77-auxiliary-support-copy-head-audit-v1.md)
- [Stage 78 support coverage split audit](docs/research/q-rope-stage78-support-coverage-split-audit-v1.md)
- [Stage 79 support-complete auxiliary copy-head audit](docs/research/q-rope-stage79-support-complete-auxiliary-copy-head-audit-v1.md)
- [Stage 80 support-routed token selector audit](docs/research/q-rope-stage80-support-routed-token-selector-audit-v1.md)
- [Stage 81 soft support-routed token selector audit](docs/research/q-rope-stage81-soft-support-routed-token-selector-audit-v1.md)
- [Stage 82 learned support-routing head audit](docs/research/q-rope-stage82-learned-support-routing-head-audit-v1.md)
- [Stage 83 nonlinear support-routing bridge audit](docs/research/q-rope-stage83-nonlinear-support-routing-bridge-audit-v1.md)
- [Stage 84 support-auxiliary pointer-generator audit](docs/research/q-rope-stage84-support-auxiliary-pointer-generator-audit-v1.md)
- [Stage 85 dual-auxiliary pointer-generator audit](docs/research/q-rope-stage85-dual-auxiliary-pointer-generator-audit-v1.md)
- [Stage 86 dual-auxiliary budget sensitivity audit](docs/research/q-rope-stage86-dual-auxiliary-budget-sensitivity-audit-v1.md)
- [Stage 87 in-decoder support-routed copy expert audit](docs/research/q-rope-stage87-in-decoder-support-routed-copy-expert-audit-v1.md)
...
For a pinned, no-provider reviewer environment, see [Reproducible review environment](docs/reproducible-environment.md):
...
Provider stacks are optional. Use `python -m pip install -e .` for the core scoring API and no-hardware review paths; use `python -m pip install -e ".[hardware]"` only for live IBM/Braket/Quandela provider work.
...
The generic provider-calibration verifier still fails with `missing-evidence` until counts are supplied for that provider packet. The completed full IBM Fez replacement run has its own real known-state calibration path in Stage 217.
...
Run the deterministic Stage 219 ranking-parity bridge:
...
| 74 | Stage 76 integrated support copy-head audit | Complete. Decision: `INTEGRATED_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; exact-offset is partially solved, but phase-cued retrieval returns to zero for every method. |
| 75 | Stage 77 auxiliary support copy-head audit | Complete. Decision: `AUXILIARY_SUPPORT_COPY_HEAD_PARTIAL_RETRIEVAL`; support supervision still does not preserve phase-cued held-out retrieval. |
| 76 | Stage 78 support coverage split audit | Complete. Decision: `SAME_SEED_SUPPORT_COVERAGE_SPLIT_EXPLAINS_STAGE77_FAILURE`; default same-seed train support has zero held-out phase-cued coverage. |
| 77 | Stage 79 support-complete auxiliary copy-head audit | Complete. Decision: `SUPPORT_COMPLETE_AUXILIARY_COPY_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support labels recover but phase-cued token retrieval still fails. |
| 78 | Stage 80 support-routed token selector audit | Complete. Decision: `SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; routing recovered support into farthest-congruent token selection solves phase-cued retrieval for every method, including `no_position`. |
| 79 | Stage 81 soft support-routed token selector audit | Complete. Decision: `SOFT_SUPPORT_ROUTED_TOKEN_SELECTOR_SOLVES_PHASE_CUED_NOT_PROMOTION`; routing learned support probabilities into farthest-congruent token selection solves phase-cued retrieval for every method, including `no_position`. |
| 80 | Stage 82 learned support-routing head audit | Complete. Decision: `LEARNED_SUPPORT_ROUTING_HEAD_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support labels recover, but a compact learned routing head does not preserve the phase-cued repair. |
| 81 | Stage 83 nonlinear support-routing bridge audit | Complete. Decision: `NONLINEAR_SUPPORT_ROUTING_BRIDGE_SUPPORT_RECOVERED_RETRIEVAL_FAILED`; support labels recover, but a nonlinear per-position routing bridge still does not preserve the phase-cued repair. |
| 82 | Stage 84 support-auxiliary pointer-generator audit | Complete. Decision: `SUPPORT_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`; in-decoder support supervision preserves capacity but does not repair held-out retrieval. |
| 83 | Stage 85 dual-auxiliary pointer-generator audit | Complete. Decision: `DUAL_AUXILIARY_POINTER_GENERATOR_WITHOUT_RETRIEVAL_GENERALIZATION`; dual support and target-attention supervision preserves train capacity but still does not repair held-out retrieval. |
| 84 | Stage 86 dual-auxiliary budget sensitivity audit | Complete. Decision: `DUAL_AUXILIARY_BUDGET_WITHOUT_RETRIEVAL_GENERALIZATION`; increasing the practical dual-auxiliary budget does not repair held-out retrieval. |
| 85 | Stage 87 in-decoder support-routed copy expert audit | Complete. Decision: `IN_DECODER_SUPPORT_ROUTED_COPY_EXPERT_SOLVES_PHASE_CUED_NOT_PROMOTION`; a structural support-routed expert repairs phase-cued retrieval for `no_position` too, while exact-offset remains below threshold. |
| 86 | Stronger decoder-only transformer | Learn held-out support-to-token routing in a stronger matched decoder path before evaluating positional-method promotion. |
| 87 | Larger or error-aware witnesses | Explore larger qubit witnesses or mitigation analysis after downstream and replication evidence justify the added complexity. |
...
The next promotion gate is documented in [Next transformer benchmark roadmap](docs/research/q-rope-next-transformer-benchmark-roadmap-v1.md). Stage 87 shows that a structural support-routed copy expert can repair phase-cued retrieval, but because `no_position` also solves and exact-offset remains below threshold, the next useful work must learn this support-to-token binding without a hand-specified expert before evaluating positional-method differences.

## Licensing and Legal Notice

Software in this repository is released under `AGPL-3.0-only`. Patent/IP status is a low-prominence legal notice only and is not part of the scientific claim; boundaries are documented in [PATENTS.md](PATENTS.md) and [Patent status note](docs/publication/patent-status-note-v1.md).
